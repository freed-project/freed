...
const PLATFORMS = [
  ...
  { id: "youtube", path: "M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" },
...
function createParticle(logo: typeof LOGOS[0], isRed: boolean): Particle {
  ...
  return {
    ...
    scale: 1,
    isRed,
    color: isRed ? COLORS.red : COLORS.blues[Math.floor(Math.random() * COLORS.blues.length)],
    startX: logo.emitX,
    startY: logo.emitY,
...
function initParticles(): Particle[] {
  return LOGOS.flatMap((logo) =>
    Array.from({ length: PARTICLES_PER_LOGO }, (_, i) => 
      createParticle(logo, i < 2) // First 2 particles per logo are red
    )
  );
}
  ...
  const tick = useCallback(() => {
    ...
    setParticles((prev) =>
      prev.map((p) => {
        ...
        if (p.state === "dead") {
          if (p.age > 60) { // Small delay before respawn
            const angleToCenter = Math.atan2(CENTER - p.startY, CENTER - p.startX);
            const spread = (Math.random() - 0.5) * 0.17; // ~±5 degrees
            const angle = angleToCenter + spread;
            return {
              ...p,
        ...
        const newX = x + vx;
        const newY = y + vy;
        const inBox = newX >= BOX.left && newX <= BOX.right && newY >= BOX.top && newY <= BOX.bottom;

        if (p.isRed) {
          // Red: attracted to center but bounces off box edge
          const dist = Math.hypot(newX - CENTER, newY - CENTER);
          
          // Gravitational pull toward center (same as blue, so they move together)
          const gravity = Math.min(0.006, 40 / (dist * dist));
          vx += (CENTER - newX) * gravity;
          vy += (CENTER - newY) * gravity;
          
          // Check if about to enter the box (bounce off edge)
          const wasOutside = x < BOX.left || x > BOX.right || y < BOX.top || y > BOX.bottom;
        ...
        return { ...p, x, y, vx, vy, opacity, scale, age, state: isDead ? "dead" : "active" };
      })
    );
    ...
    <div className="relative w-full aspect-square">
      <svg viewBox="-50 -50 500 500" className="w-full h-full">
        ...
        <motion.circle
          ...
          fill="none" stroke="url(#ringGradient)" strokeWidth="1" strokeDasharray="15 8"
          animate={{ rotate: 360 }}
        ...
        {[0, 72, 144, 216, 288].map((angle) => (
          <motion.line
            key={angle}
            x1={CENTER} y1={CENTER} x2={CENTER} y2={CENTER - RING_RADIUS}
        ...
        {LOGOS.map((logo) => (
          <g key={logo.id}>
            <motion.circle
              cx={logo.cx} cy={logo.cy} r="22" strokeWidth="2" filter="url(#glow)"
              animate={{
                fill: [`${COLORS.logoBlue}20`, `${COLORS.logoBlue}20`, `${COLORS.logoBlue}20`, `${COLORS.red}20`, `${COLORS.logoBlue}20`],
                stroke: [`${COLORS.logoBlue}50`, `${COLORS.logoBlue}50`, `${COLORS.logoBlue}50`, `${COLORS.red}50`, `${COLORS.logoBlue}50`],
              }}
              transition={{ duration: 5, repeat: Infinity, delay: logo.flashDelay, times: [0, 0.7, 0.8, 0.85, 1] }}
            />
            <motion.g
              animate={{ rotate: 360 * logo.spinDirection }}
              transition={{ duration: logo.spinDuration, repeat: Infinity, ease: "linear" }}
              style={{ transformOrigin: `${logo.cx}px ${logo.cy}px` }}
            >
              <g transform={`translate(${logo.cx - 10}, ${logo.cy - 10}) scale(${20 / 24})`}>
                <motion.path
                  d={logo.path}
                  animate={{ fill: [COLORS.logoBlue, COLORS.logoBlue, COLORS.logoBlue, COLORS.red, COLORS.logoBlue] }}
                  transition={{ duration: 5, repeat: Infinity, delay: logo.flashDelay, times: [0, 0.7, 0.8, 0.85, 1] }}
                />
              </g>
            </motion.g>
        ...
        <motion.circle
          ...
          fill="url(#centerGradient)" opacity="0.08" filter="url(#glow)"
          animate={{ r: [55, 62, 55], opacity: [0.08, 0.15, 0.08] }}
          transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
        />

        {/* Center box */}
        <rect x="155" y="155" width="90" height="90" rx="16" fill="rgba(0,0,0,0.4)" />
        <rect x="155" y="155" width="90" height="90" rx="16" fill="none" stroke="url(#centerGradient)" strokeWidth="3" filter="url(#glow)" />
        <text x={CENTER} y="215" textAnchor="middle" fill="white" fontSize="48" fontWeight="bold" fontFamily="system-ui">F</text>

        {/* Pulse rings */}
        {[1, 2, 3].map((i) => (
          <motion.rect
            key={i}
            x="155" y="155" width="90" height="90" rx="16"

...
const phases: Phase[] = [
  ...
  },
  {
    ...
    description:
      "A people CRM with a canonical Person plus Account identity graph, a Pixi WebGL Friends workspace with theme-aware graph roles, bounded D3 Force worker layout for the person field, single-pass activity indexing, bucketed overlap resolution, interactive label degradation during pan and zoom, confirmed friend hubs near the center, provisional human identities in the middle field, linked channels orbiting people, unlinked channels in provider islands around the edge, RSS treated as a normal provider island, followed social channels searchable by name or handle from the primary feed search with profile commands for Friends, Map, and relationship promotion, retained scene-object caching so pan and zoom stay responsive, persisted drag-to-pin placement that keeps drag-to-link intact, expanded showcase and stress sample graphs, Google Contacts imports in Freed Desktop that create friend people by default, server-proxied refreshable Google credentials for long-running Contacts sync, PWA status visibility for synced contacts, suggestion-only same-person review across platforms, AI-ranked suggested friends from local identity and content signals, Followed, Friends, and Fam relationship controls over the existing care-level model, a collapsible desktop details rail with a compact floating selection card when the rail is closed plus click-off dismissal on empty graph space, faster graph-native pinch zoom that stays inside the workspace on Safari, and a mobile `Details` mode in the shared toolbar while the map resolves identity through linked accounts with unified header controls plus current, future, and past filters and a quieter lower-left timeline scrubber for replay.",
    status: "current",
    planLink:

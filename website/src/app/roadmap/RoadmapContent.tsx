...
const phases: Phase[] = [
  ...
  },
  {
    ...
    description:
      "Save any URL from Freed Desktop or mobile. Full offline reading with layered content cache, saved-item pinning, post-save reader handoff, on-demand reader hydration, AI summarization, and tag navigation.",
    status: "complete",
    planLink:
  ...
  },
  {
    ...
    title: "Sync Layer",
    description:
      "Local relay and Google Drive sync are working, with server-proxied Google token exchange, a production proxy fallback for Freed Desktop builds, dev and preview PWA callback relay, appDataFolder Drive polling, durable Google OAuth refresh, recoverable Google Contacts token-refresh failures, initial Drive download recovery after token refresh, merged-upload local convergence, destructive Automerge merge blocking, PWA local-change Drive uploads, document-init-gated PWA cloud startup, mobile-safe Drive upload bodies, cloud health diagnostics, visible transfer diagnostics, manual Drive sync in Freed Desktop and PWA Settings, recent activity timelines, retry and reconnect actions, debug charts, the timed no-cloud-sync launch banner, and desktop snapshot restore. Dropbox remains gated while its provider work finishes, and iCloud remains the open item.",
    status: "current",
    planLink:
  ...
  },
  {
    ...
    description:
      "Freed Desktop ships for macOS (ARM + Intel), Windows, and Linux with signed auto-updates, signed and notarized macOS releases, a Windows signing plan scaffolded around Microsoft Artifact Signing, a Freed-owned updater endpoint, launch-time and background update checks, local production or dev release-channel switching that keeps dev installs on the newest eligible build, first-run legal gating, provider risk interstitials, filtered inner Settings list panels for late-loaded content, visible cloud transfer diagnostics in Sync settings, manual Drive sync, cloud activity timelines, a top-toolbar background activity monitor with elapsed active-work timers, cloud uploads that wait behind active outbox and social-scrape work before retrying, native terminal sync soaks for dev-channel installed builds with renderer-recovery retry, quiet installed startup that keeps the main window non-focusable through startup probes, a production-default Google token proxy fallback, recoverable Google Contacts auth failures in Settings, a permanent local social media vault outside Automerge, browser-style back and forward shortcuts, a device-local OS-wide Save Content shortcut with clipboard URL prefill and post-save reader handoff, draggable shared top-toolbar chrome, a tri-state resizable source sidebar with compact icon rail and floating compact search, synced global animation intensity controls, local snapshot rollback for disaster recovery, a native startup recovery window outside the React tree with in-place update install plus channel-aware browser download fallback, a provider health dashboard, failing-feed unsubscribe tools, renderer heartbeat diagnostics in the local log, runtime memory telemetry for long-session leak hunting, Freed-owned WebKit RSS attribution, adaptive social scrape memory preflight cleanup with scraper recycle verification, renderer heap and DOM diagnostics, renderer recovery safe mode, reclaimable WebKit RSS tail handling that still recovers hot high-RSS renderers and CPU-hot multi-GB WebKit growth, bounded deep WebKit diagnostics, bounded scheduled RSS refresh, reduced graphics pressure under memory stress, reduced worker payload churn during large-library updates, duplicate-fetch suppression, single-worker randomized fetch pacing with capped backoff for slow AI or network work, fetcher rescans gated to real library growth, incremental Automerge persistence compaction, queued native persistence and sync side effects for hot UI paths, event-aware outbox patch drains, incremental item-patch UI state updates, incremental RSS feed metadata patches, visible-scope bulk archive for read cleanup, leaner outbox retry bookkeeping, dead-feed health cleanup, capped preserved-text previews with on-demand reader hydration, density-aware fixed-height feed rows, matching loading skeletons, and media thumbnails that do not remeasure while scrolling, reader rail media thumbnails, story cards that morph into the reader rail like regular cards, preview rail toggles that animate open and closed unless animations are set to none, X reply threads with explicit beta inline loading, Facebook group rows with single-line leave verification, Facebook and Instagram post comments behind explicit beta inline loading, saved-item cache pinning, labeled native local previews with seeded sample data and local legal gates accepted, reviewed cumulative release headings, paginated changelog history, compact changelog previews in Settings updates, a dev-to-main production promotion guard that blocks stale releases, public-safe bug reporting with clearer private bundle gating, and browser preview settings that no longer crash on native-only sections or background social refresh.",
    status: "current",
    priority: true,
    planLink:
  ...
  },
  {
    ...
    title: "PWA Reader",
    description:
      "Primary mobile surface with first-run legal gating, URL-backed view state, pinned saved-reader caching, warmed offline article plus image caching, local offline cache modes, status-only source settings for synced Feeds, social providers, and Google Contacts while Freed Desktop owns setup, visible cloud sync diagnostics in Settings with manual Drive sync, pending-upload explanations, recent activity, local-change Drive uploads when no LAN relay is connected, document-init-gated Drive and relay resume, compact stacked mobile settings, balanced mobile toolbar controls, viewport-fixed reader action menus, homescreen install prompts for browser and iOS Safari flows, public-safe bug reporting with clearer private bundle controls, and switchable production or dev update channels across `app.freed.wtf` and the `dev` branch lane at `dev-app.freed.wtf`.",
    status: "complete",
    planLink:
  ...
  },
  {
    ...
    description:
      "Facebook and Instagram integrated via Tauri WebView scraping, with suggested-post filtering, long-text expansion, source-level post and story filters, stricter Instagram story viewer validation, hardened playable story capture, Instagram media-key duplicate repair, same-platform story duplicate repair, silent background media guarding, serialized background scraper sessions, shared memory-preflight backoff across social providers, transient memory-pressure health recovery, memory-aware scrape pass planning, a local social scrape optimization loop, filtered Facebook group controls with stored-name repair, ID-tail fallback labels, rendered-card title extraction, missing-name page hydration with row-level progress, single-group leave verification, explicit risk-gated joined-groups refresh, provider health summaries, Meta export import, a permanent local archive for the user's own media, a gated Story Wall settings section grouped with AI, media-backed previews, live style controls, Instagram archive export guidance, GitHub Pages publishing, explicit reply links plus beta inline reply loading for X, Facebook, and Instagram reader posts, private story reply states, and smart backoff when sync looks rate-limited.",
    status: "current",
    planLink:
  ...
  },
  {
    ...
    description:
      "A people CRM with a canonical Person plus Account identity graph, a summary-backed Friends graph atlas with capped visible nodes, worker-built level-of-detail tiers, a theme-aware Three.js starfield renderer, provider nebula and ring constellations, Troika billboard labels, transform-only pan and pinch, confirmed friend stars closer to the camera, provisional human identities in the middle field, linked channels packed into tight fields around people with short bright connectors, unlinked channels around the edge, RSS treated as a normal provider island, followed social channels searchable by name or handle from the primary feed search with profile commands for Friends, Map, and relationship promotion, context-menu pinning and account linking through a searchable person picker, expanded showcase and stress sample graphs with future-dated map fixtures, Google Contacts imports in Freed Desktop that create friend people by default, server-proxied refreshable Google credentials for long-running Contacts sync, PWA status visibility for synced contacts, suggestion-only same-person review across platforms, AI-ranked suggested friends from local identity and content signals, Followed, Friends, and Fam relationship controls over the existing care-level model, a full-canvas Friends starfield that fills under the primary app sidebar while fitting content to the visible workspace, a collapsible desktop details rail with a compact floating selection card when the rail is closed plus click-off dismissal on empty graph space, workspace-native pinch zoom that stays inside the graph on Safari, and a mobile `Details` mode in the shared toolbar while the map resolves identity through linked accounts with unified header identity controls plus an always-visible lower-left time range slider, handle-following labels, and `all time`, `today`, and `last week` presets for historical, current, and future location windows.",
    status: "current",
    planLink:

  ...
  gdrive: {
    ...
    icon: (
      <svg className="theme-icon-media h-6 w-6 flex-shrink-0" viewBox="0 0 87.3 78" fill="currentColor">
        ...
        <path d="M43.65 25L29.9 1.2C28.55 2 27.4 3.1 26.6 4.5L1.2 48.5A9.06 9.06 0 000 53h27.5z" opacity="0.86" />
        <path d="M73.55 76.8c1.35-.8 2.5-1.9 3.3-3.3l1.6-2.75 7.65-13.25c.8-1.4 1.2-2.95 1.2-4.5H59.8l5.85 11.2z" opacity="0.94" />
        <path d="M43.65 25L57.4 1.2C56.05.4 54.5 0 52.9 0H34.4c-1.6 0-3.15.45-4.5 1.2z" opacity="0.72" />
        <path d="M59.8 53H27.5L13.75 76.8c1.35.8 2.9 1.2 4.5 1.2h50.8c1.6 0 3.15-.4 4.5-1.2z" opacity="0.8" />
        <path d="M73.4 26.5l-12.65-21.9c-.8-1.4-1.95-2.5-3.3-3.3L43.65 25 59.8 53h27.45c0-1.55-.4-3.1-1.2-4.5z" opacity="0.64" />
      </svg>
    ),
  },
};
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {isConnected && (
            <span className="theme-feedback-text-success inline-flex items-center gap-1 text-xs">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-[rgb(var(--theme-feedback-success-rgb))]" />
              Connected
            </span>
          )}
        </div>
        <p className="mt-0.5 text-xs text-[var(--theme-text-muted)]">{meta.detail}</p>
        {error && <p className="theme-feedback-text-danger mt-1 text-xs">{error}</p>}
      </div>

      {isConnected && onDisconnect ? (
        <button
          onClick={(e) => { e.stopPropagation(); onDisconnect(provider); }}
          className="flex-shrink-0 rounded-lg bg-[var(--theme-bg-muted)] px-2.5 py-1 text-xs text-[var(--theme-text-muted)] transition-colors hover:bg-[rgb(var(--theme-feedback-danger-rgb)/0.1)] hover:text-[rgb(var(--theme-feedback-danger-rgb))]"
        >
          Disconnect
        </button>
      ) : (

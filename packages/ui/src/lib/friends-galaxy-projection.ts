export interface FriendsGalaxyViewportProjection {
  viewProjection: ArrayLike<number>;
  width: number;
  height: number;
}

export function projectFriendsGalaxyWorldPoint(
  target: Float32Array,
  ...
  const clipX = matrix[0]! * worldX + matrix[4]! * worldY +
    matrix[8]! * worldZ + matrix[12]!;
  const clipY = matrix[1]! * worldX + matrix[5]! * worldY +
    matrix[9]! * worldZ + matrix[13]!;
  const clipW = matrix[3]! * worldX + matrix[7]! * worldY +
    matrix[11]! * worldZ + matrix[15]!;

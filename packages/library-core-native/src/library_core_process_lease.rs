...
//! process is allowed to act as the Library runtime. Freed Desktop and a
...
/// Host identity written into an attributable process-lease refusal.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ProcessLeaseIdentity<'a> {
    package: &'a str,
    version: &'a str,
}

impl<'a> ProcessLeaseIdentity<'a> {
    pub const fn new(package: &'a str, version: &'a str) -> Self {
        Self { package, version }
    }
}

/// Clock used only for the bounded refusal diagnostic.
pub trait ProcessLeaseClock {
    fn now_ms(&self) -> u64;
}

struct SystemProcessLeaseClock;

impl ProcessLeaseClock for SystemProcessLeaseClock {
    fn now_ms(&self) -> u64 {
        SystemTime::now()
...
impl LibraryCoreProcessLease {
    ...
    pub fn acquire(
        ...
        Self::acquire_with_clock(requested_data_root, identity, &SystemProcessLeaseClock)
    }

    /// Acquire the lease with an injected diagnostic clock.
...
struct ProcessRefusalDiagnostic<'a> {
    format: &'static str,
    recorded_at_ms: u64,
    requester_pid: u32,
    holder_pid: Option<u32>,
    requester_executable: Option<String>,
    requester_package: &'a str,
    requester_version: &'a str,
    data_root: String,
    lock_path: String,
    reason: &'static str,
}
...
mod tests {
    ...
    const TEST_IDENTITY: ProcessLeaseIdentity<'static> =
        ProcessLeaseIdentity::new(env!("CARGO_PKG_NAME"), env!("CARGO_PKG_VERSION"));
    ...
    fn lease_helper_process() {
        ...
        match LibraryCoreProcessLease::acquire(&data_root, TEST_IDENTITY) {
            Ok(lease) => {
    ...
    #[test]
    fn refusal_diagnostic_uses_the_injected_clock_and_host_identity() {
        struct FixedClock;

        impl ProcessLeaseClock for FixedClock {
            fn now_ms(&self) -> u64 {
                1_700_000_000_123
            }
        }
        ...
        let holder =
            LibraryCoreProcessLease::acquire(&data_root, TEST_IDENTITY).expect("acquire holder");
        let identity = ProcessLeaseIdentity::new("headless-test-host", "7.8.9");
        let error =
            match LibraryCoreProcessLease::acquire_with_clock(&data_root, identity, &FixedClock) {
                Ok(_) => panic!("second holder must be refused"),
                Err(error) => error,
            };
        assert!(matches!(error, LibraryCoreProcessLeaseError::Held { .. }));

        let diagnostic: serde_json::Value = serde_json::from_slice(
            &std::fs::read(data_root.join(PROCESS_REFUSAL_DIAGNOSTIC_FILE))
                .expect("read refusal diagnostic"),
        )
        .expect("parse refusal diagnostic");
        assert_eq!(diagnostic["recordedAtMs"], 1_700_000_000_123_u64);
        assert_eq!(diagnostic["requesterPackage"], "headless-test-host");
        assert_eq!(diagnostic["requesterVersion"], "7.8.9");
        drop(holder);
    }
    ...
    fn lease_control_files_never_create_or_transport_sqlite_sidecars() {
        ...
        let data_root = temp.path().join("library-core");
        let lease =
            LibraryCoreProcessLease::acquire(&data_root, TEST_IDENTITY).expect("acquire lease");

...
pub const SQLITE_QUERY_PROGRAMS: &[SqliteQueryProgram] = &[
    ...
    SqliteQueryProgram { query_id: "rss_item_summary_v1", maximum_scan_rows: 1, sql: "SELECT sum(totalCount) AS totalCount, sum(unreadCount) AS unreadCount FROM (SELECT count(*) AS totalCount, count(*) - count(read_at) AS unreadCount FROM library_feed_items WHERE rss_feed_url IS NOT NULL UNION ALL SELECT count(*) AS totalCount, count(*) - count(read_at) AS unreadCount FROM library_feed_items WHERE platform = 'rss' AND rss_feed_url IS NULL);", reverse_sql: None, count_sql: "SELECT 1;", variants: &[

...
fn query_feed_page(
    ...
    let (generation_id, source_revision): (String, i64) = transaction.query_row(
        "SELECT library_id, source_revision FROM library_meta WHERE singleton_id = 1;",
        [],
        |row| Ok((row.get(0)?, row.get(1)?)),
    )?;
    if !valid_lower_hex_64(&generation_id) || !valid_safe_integer(source_revision) {
        return Err(invalid("normalized feed query source identity is invalid"));
    }

//! Native verification boundary for canonical normalized operation envelopes.
//!
//! This module owns protocol reconstruction, digest and actor-chain validation,
//! Ed25519 verification, and sealed verified transaction construction. Storage
//! callers can commit its output, but no renderer value can manufacture it.
...
use crate::normalized_operation::{ActorState, VerifiedOperation, VerifiedOperationTransaction};
...
pub(crate) fn digest_hex(domain: &str, value: &Value, index: usize) -> LibraryCoreResult<String> {
...
pub(crate) fn parse_causal_tips(
    value: &Value,
    index: usize,

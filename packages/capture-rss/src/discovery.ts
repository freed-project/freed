...
const PLATFORM_PATTERNS: PlatformPattern[] = [
  ...
  // YouTube - Channel
  {
    ...
    transform: (url, match) => `https://www.youtube.com/feeds/videos.xml?channel_id=${match[2]}`
  },
  // YouTube - User
  {
    match: /^https?:\/\/(www\.)?youtube\.com\/user\/([^\/]+)/,
    transform: (url, match) => `https://www.youtube.com/feeds/videos.xml?user=${match[2]}`
  },
  ...
  // GitHub - Releases
  {
    ...
    transform: (url, match) => `https://github.com/${match[2]}/${match[3]}/releases.atom`
  },
  // GitHub - Commits
  {
    match: /^https?:\/\/(www\.)?github\.com\/([^\/]+)\/([^\/]+)\/commits/,
    transform: (url, match) => `https://github.com/${match[2]}/${match[3]}/commits.atom`
  },
  ...
  // Blogger
  {
    ...
    transform: (url, match) => `https://${match[1]}.blogspot.com/feeds/posts/default`
  },
 ...
 */
function extractAllFeedsFromHtml(html: string, baseUrl: string): DiscoveredFeed[] {
  const feeds: DiscoveredFeed[] = []
  
  // Match <link> tags with feed types
  const linkPattern = /<link[^>]+type=["'](application\/(rss|atom)\+xml|application\/feed\+json)["'][^>]*>/gi
  const hrefPattern = /href=["']([^"']+)["']/i
  ...
  for (const match of matches) {
    ...
    if (hrefMatch) {
      try {
        ...
        const type = match[1].includes('atom') ? 'atom' : 
                    match[1].includes('json') ? 'json' : 'rss'
        feeds.push({
          url,
          title: titleMatch?.[1],
          type

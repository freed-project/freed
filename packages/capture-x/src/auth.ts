...
function getCookiePath(browser: SupportedBrowser): string | null {
  ...
  const paths: Record<SupportedBrowser, Record<string, string>> = {
    chrome: {
      darwin: join(home, 'Library/Application Support/Google/Chrome/Default/Cookies'),
      linux: join(home, '.config/google-chrome/Default/Cookies'),
      win32: join(process.env.LOCALAPPDATA || '', 'Google/Chrome/User Data/Default/Network/Cookies')
    },
    firefox: {
    ...
    edge: {
      darwin: join(home, 'Library/Application Support/Microsoft Edge/Default/Cookies'),
      linux: join(home, '.config/microsoft-edge/Default/Cookies'),
      win32: join(process.env.LOCALAPPDATA || '', 'Microsoft/Edge/User Data/Default/Network/Cookies')
    },
    brave: {
      darwin: join(home, 'Library/Application Support/BraveSoftware/Brave-Browser/Default/Cookies'),
      linux: join(home, '.config/BraveSoftware/Brave-Browser/Default/Cookies'),
      win32: join(process.env.LOCALAPPDATA || '', 'BraveSoftware/Brave-Browser/User Data/Default/Network/Cookies')
    }
  }
 ...
 */
async function decryptChromeCookie(encryptedValue: Buffer): Promise<string | null> {
 ...
 */
async function extractChromiumCookies(cookiePath: string): Promise<XCookies | null> {
  try {
    ...
    for (const row of rows) {
      if (row.name === 'ct0') {
        ...
        authToken = row.value || await decryptChromeCookie(row.encrypted_value!)
      }
    }
 ...
 */
async function extractFirefoxCookies(cookiePath: string): Promise<XCookies | null> {
  try {
    const db = new Database(cookiePath, { readonly: true })
 ...
 */
export async function extractCookies(browser: SupportedBrowser = 'chrome'): Promise<XCookies | null> {
  if (browser === 'firefox') {
 ...
 */
export async function extractCookiesAuto(): Promise<{ cookies: XCookies; browser: SupportedBrowser } | null> {

import { CONTENT_SIGNAL_VERSION } from "./content-signals.js";
import type { Account, ContentSignal, FeedItem, Person, RssFeed, SampleDataFingerprint } from "./types.js";
...
const classificationSignals = {
  inspiring: "essay",
  event: "event",
  personal: "life_update",
  conversation: "discussion",
  news: "news",
} as const satisfies Readonly<Record<NonNullable<typeof acceptedArcs[number]["episodes"][number]["classification"]>, ContentSignal>>;

export function sampleEditorialContentSignals(
  classification: NonNullable<typeof acceptedArcs[number]["episodes"][number]["classification"]>,
  inferredAt: number,
): NonNullable<FeedItem["contentSignals"]> {
  const signal = classificationSignals[classification];
  return {
...
export function generateEditorialSampleData(options: {
  ...
  for (const arc of orderedArcs) {
    ...
    for (const episode of arc.episodes) {
      if (!episode.classification) {
        throw new Error(`Missing editorial classification for ${arc.characterId}: ${episode.title}`);
      }
      ...
      const item: FeedItem = {
        ...
        contentSignals: sampleEditorialContentSignals(episode.classification, now),
  ...
  if (top && top.globalId === options.previousTopItemId) {

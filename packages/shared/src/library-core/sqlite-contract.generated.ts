...
export const LIBRARY_CORE_NORMALIZED_SCHEMA_SHA256 = "d854cc9e4401dc0e0f605c28be30995cdbca6e6273d37d05458c61d01b3bc788" as const;
...
export const LIBRARY_CORE_SQLITE_CHECKPOINT_IMPORT_PROGRAMS = {
  ...
  "32_person_reach_out": {
    "hasChunkBytes": false,
    "primaryKeyArity": 2,
    "sql": "INSERT INTO library_person_reach_outs (person_id, ordinal, channel, logged_at, notes) SELECT json_extract(?1, '$[0]'), json_extract(?1, '$[1]'), json_extract(?2, '$.channel'), json_extract(?2, '$.loggedAt'), json_extract(?2, '$.notes');"
  },
  "40_account": {
    "hasChunkBytes": false,
    "primaryKeyArity": 1,
...
export const LIBRARY_CORE_SQLITE_MUTATION_PROGRAMS = {
  ...
  "person_upsert": {
    ...
    "dependentDeleteSql": [
      ...
      "DELETE FROM library_person_reach_outs WHERE person_id = ?1;"
    ],
    "dependentInsertSql": [
      "INSERT OR IGNORE INTO library_person_tags (person_id, tag) SELECT ?1, value FROM json_each(?2, '$.tags');",
      "INSERT INTO library_person_reach_outs (person_id, ordinal, logged_at, channel, notes) SELECT ?1, CAST(key AS INTEGER), json_extract(value, '$.loggedAt'), json_extract(value, '$.channel'), json_extract(value, '$.notes') FROM json_each(?2, '$.reachOutLog');"
    ],
    "entityType": "Person",

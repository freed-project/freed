...
export const LIBRARY_CORE_LOCAL_AUTHORITY_REGISTRY = [
  ...
  },
  {
    ...
    soleOwner: "Library Core SQLite device graph layout tables",
    locality: "device-local",
    role: "active-authority",
    authoritative: true,
    physicalStores: [{
      ...
      locator: "Library Core SQLite",
      keys: [
        "library_device_graph_layout_state",
        "library_device_person_graph_layout",
        "library_device_account_graph_layout",
      ],
    }],
    retention: {
      kind: "bounded-by-rule",
      rules: [
        { scope: "person", limit: 1, unit: "row per synchronized Person" },
        { scope: "account", limit: 1, unit: "row per synchronized Account" },
      ],
    },
    backup: "exclude-device-local",
    export: "exclude",
    redaction: "redact-sensitive-fields",
    resetSemantics: "Entity deletion cascades its local row. Factory reset deletes the SQLite Library and the retired localStorage import key.",
    snapshot: "excluded",
    migration: "migrate-to-library-core",
    cutover: {
      blocksCutover: false,
      reason: "Graph pages join foreign-keyed device-local SQLite rows, and pin actions use the same typed mutation contract in Desktop and PWA. Startup imports the retired localStorage record once and deletes it.",
    },
    sourceReferences: [
      "packages/ui/src/lib/device-graph-layout.ts",
      "packages/library-core-native/src/device_graph_layout.rs",
      "packages/pwa/src/lib/library-core-sqlite-engine.ts",
  ...
  },
  {
    ...
    export: "exclude",
    redaction: "not-applicable",
    resetSemantics: "Successful migration confirmation removes that revision's spool and scratch graph. Factory reset removes every registered native derived-runtime root. Clearing site data removes local derived-runtime controls.",
    snapshot: "rebuildable",
    migration: "rebuild-after-cutover",
    cutover: {
      blocksCutover: false,

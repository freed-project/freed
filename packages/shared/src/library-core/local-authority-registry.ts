...
export const LIBRARY_CORE_LOCAL_AUTHORITY_REGISTRY = [
  ...
  {
    registryKey: "library-core-media-blob-store",
    soleOwner: "packages/desktop/src-tauri/src/library_core_media_recovery.rs",
    locality: "blob",
    role: "backup",
    authoritative: false,
    physicalStores: [{
      kind: "filesystem",
      platforms: ["desktop"],
      locator: `${APP_DATA}/library-core/media-blobs-v1`,
      keys: ["<digest-prefix>/<blob-content-digest>"],
    }],
    retention: {
      kind: "unbounded-current",
      reason: "The first recovery slice retains every content-addressed legacy media copy and deliberately has no deletion path.",
    },
    backup: "include-private",
    export: "include-private",
    redaction: "retain-private-bytes",
    resetSemantics: "This recovery owner never deletes a blob. Removing the Freed application-data root outside this owner removes the store.",
    snapshot: "legacy-recovery",
    migration: "retain-legacy-recovery",
    cutover: {
      blocksCutover: false,
      reason: "These immutable bytes are a verified recovery backup. They do not replace the legacy media vault or grant Library Core write authority.",
    },
    sourceReferences: [
      "docs/LIBRARY-CORE-CONTRACT.md",
      "packages/desktop/src-tauri/src/library_core_media_recovery.rs",
    ],
  },
  {
    registryKey: "library-core-media-vault-reconciliation",
    soleOwner: "packages/desktop/src-tauri/src/library_core_media_recovery.rs",
    ...
    physicalStores: [{
      kind: "filesystem",
      platforms: ["desktop"],
      locator: `${APP_DATA}/library-core/media-vault-reconciliation-v1`,
      keys: [
        "source-manifests/<source-manifest-digest>.json",
        "logical-backups/<source-manifest-digest>.json",
        "receipts/<source-manifest-digest>.json",
        "staging/<temporary-file>",
      ],
    }],
    retention: {
      kind: "unbounded-current",
      reason: "Source evidence, logical backups, and reconciliation receipts remain durable so restart cannot reinterpret a legacy source.",
    },
    backup: "include-private",
    export: "include-private",
    redaction: "retain-private-bytes",
    resetSemantics: "This recovery owner never deletes reconciliation evidence. Removing the Freed application-data root outside this owner removes it.",
    snapshot: "legacy-recovery",
    migration: "retain-legacy-recovery",
    cutover: {
      blocksCutover: false,
      reason: "The receipt proves a local recovery attempt only. It grants no elected migration, actor, provider, or cloud authority.",
    },
    sourceReferences: [
      "docs/LIBRARY-CORE-CONTRACT.md",
      "packages/desktop/src-tauri/src/library_core_media_recovery.rs",
...
export const LIBRARY_CORE_LOCAL_AUTHORITY_SOURCE_OWNERS = [
  ...
  {
    registryKey: "library-core-media-blob-store",
    sourcePath: "packages/desktop/src-tauri/src/library_core_media_recovery.rs",
    sourceTokens: [
      'const BLOB_DIRECTORY: &str = "media-blobs-v1"',
      "const BLOB_DIGEST_PREFIX: &[u8] =",
    ],
    registeredKeys: ["<digest-prefix>/<blob-content-digest>"],
  },
  {
    registryKey: "library-core-media-vault-reconciliation",
    sourcePath: "packages/desktop/src-tauri/src/library_core_media_recovery.rs",

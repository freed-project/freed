...
const CLOSED_ACTOR_ENROLLMENT_BODY = Symbol(
  "closed Library Core actor enrollment body",
);
...
export function constructLibraryCoreActorEnrollmentBodyV1(
  ...
  const actorPublicKeyCandidate = readInput(record, "actor_public_key");
  if (!isLibraryCoreEd25519PublicKeyHex(actorPublicKeyCandidate)) {
    throw new TypeError(
      "actor_public_key must be 64 lowercase hexadecimal characters",
    );
  }
  const libraryId = requireHex64(readInput(record, "library_id"), "library_id");
  const installationIncarnation = requireHex64(
    readInput(record, "installation_incarnation"),
    "installation_incarnation",
  );
  const actorIncarnationNonce = requireHex64(
    readInput(record, "actor_incarnation_nonce"),
    "actor_incarnation_nonce",
  );
  const epochCandidate = readInput(record, "epoch");
  if (
  ...
  const createdAtCandidate = readInput(record, "created_at_ms");
  ...
  const operationIdCandidate = readInput(record, "operation_id");
  ...
  const actorPublicKeyFingerprint = digest(dependencies, "actor-public-key", {
    signature_algorithm: "ed25519",
    actor_public_key: actorPublicKeyCandidate,
  });
  const actorId = digest(dependencies, "actor-id", {
  ...
  const body = Object.freeze({
    ...
    epoch_id: requireHex64(readInput(record, "epoch_id"), "epoch_id"),
    schema_version: 1,
    authority_key_id: requireHex64(
      readInput(record, "authority_key_id"),
      "authority_key_id",
    ),
    installation_incarnation: installationIncarnation,
    actor_incarnation_nonce: actorIncarnationNonce,
    ...
    observed_frontier: snapshotLibraryCoreCausalFrontier(
      readInput(record, "observed_frontier"),
      "observed_frontier",
    ),

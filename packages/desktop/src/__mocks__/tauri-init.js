...
export function tauriInitScript() {
  return `(function () {
    // The test init script may already have installed handlers and overrides.
    // Loading preview HTML must never reset that state.
    if (window.__TAURI_MOCK_BOOTSTRAPPED__) return;
    window.__TAURI_MOCK_BOOTSTRAPPED__ = true;
    ...
    function sqliteNormalizedQuery(args) {
      ...
      // Browser previews have no native background content-fetch queue.
      if (request.queryId === 'content_fetch_claim_v1') {
        return {
          nextCursor: null,

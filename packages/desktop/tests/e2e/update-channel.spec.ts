...
test("updates settings shows recent changelog entries and opens the full changelog", async ({
  ...
  await expect(preview.getByText("v26.5.309")).toBeVisible();
  await expect(preview.getByText("v26.5.610-dev")).toHaveCount(0);
  ...
  await expect(preview.getByRole("article")).toHaveCount(5);
  await expect(preview.getByText("v26.5.610-dev")).toBeVisible();
  await expect(preview.getByText("Freed Desktop cuts renderer memory pressure and isolates social scraper WebKit storage")).toBeVisible();
  await page.evaluate(() => {

...
2. Review `source.libraryCoreActivation` in the generated release JSON before
   ...
   Publish the release PR as a draft with the helper in step 5. Then generate
   the canonical approval text with
   `node scripts/library-core-release-activation.mjs approval-comment --artifact=release-notes/releases/<tag>.json --pull=<number>`.
   The owner must post that byte-identical text as their own comment on that
   PR. Record the authenticated decision with
   `node scripts/library-core-release-activation.mjs record-owner-approval --artifact=release-notes/releases/<tag>.json --comment-url=<canonical-comment-url>`.
   The command writes only the verified decision fields after reloading the
   exact PR head and owner comment. Commit that change, run the normal release
   validation from step 3, and only then make the PR ready.

   The proposal digest covers the complete release JSON while normalizing only
   the decision field. Validation reloads the release JSON from the referenced
   PR head and rejects any other changed release field, fork, wrong lane, or
   closed-unmerged PR. Never post the comment for the owner, invent or reuse its
   URL, accept offline copied comment data, hand-edit the decision fields, or
   confuse release authority with runtime activation authority.
...
5. Publish a no-transition release-prep branch with `./scripts/worktree-publish.sh --base main --ready --title "chore: prepare v<version>"` for production, or use `--base dev` for dev. For a `review_required` Library Core transition, omit `--ready` for the first publication after its draft preflight, complete the exact owner-comment flow in step 2, commit only the command-generated decision fields, rerun exact-head validation, then publish the same PR with `--ready`. A deliberately provisioned unattended host may wrap this helper with `FREED_TRUSTED_PUBLISHER`. Never push a release commit directly to `dev` or `main`.

...
async function addFeedByUrl(url: string): Promise<void> {
  ...
  if (!feedUrl) {
    console.error('Could not find RSS feed. Try providing the direct feed URL.')
    process.exit(1)
  }
...
async function syncAllFeeds(): Promise<void> {
  ...
  // Fetch all feeds
  const results = await fetchFeeds(feedRequests, config['capture-rss'].concurrency)
...
function showRecent(count: number = 10): void {
  ...
  for (const item of items) {
    ...
    const text = item.content.text?.slice(0, 100) || item.content.linkPreview?.title || '(no text)'

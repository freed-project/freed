...
test("a synthetic alarm aggregate and failed verdict produce issue-backed evidence", () => {
  const alarms = {
  ...
  assert.match(
    rendered,
    /^Canonical issue: https:\/\/github\.com\/freed-project\/freed\/issues\/1066$/m,
  );
  assert.doesNotMatch(rendered, /Program task:/);
...
test("renderEvidenceCandidate links the canonical debt issue", () => {
  const candidate = {
    ...
    githubIssueUrl: "https://github.com/freed-project/freed/issues/1070",
    evidenceFingerprint: "f".repeat(64),
    sourceHealth: "healthy",
  };
  const text = renderEvidenceCandidate(
    candidate,
    2,
  ...
  assert.match(lines[2], /^Generated by scripts\/triage\.mjs/);
  assert.match(
    text,
    /^Canonical issue: https:\/\/github\.com\/freed-project\/freed\/issues\/1070$/m,
  );
  assert.ok(
...
test("nightly planner loads only current issue-backed triage evidence", () => {
  const dir = mkdtempSync(path.join(os.tmpdir(), "freed-triage-nightly-"));
  const ranked = buildCandidates({
    alarms: { cloud_loop: attributableAlarm(7, NOW) },
    verdictInfo: null,
    canaryInfo: null,
    ciIssues: [],
    nowMs: NOW,
  });
  const [filePath] = emitCandidates(ranked, dir, {
    nowIso: new Date(NOW).toISOString(),
  });
  const loaded = loadTriageCandidates(dir, NOW + 60_000);
  assert.equal(loaded.length, 1);
  assert.equal(loaded[0].bucketId, "cloud-loop");
  assert.equal(loaded[0].fresh, true);
  assert.equal(loaded[0].issueKind, "debt");
  assert.equal(loaded[0].githubIssueNumber, 1066);
  assert.equal(
    loaded[0].githubIssueUrl,
    "https://github.com/freed-project/freed/issues/1066",
  );
  ...
  assert.equal(triage.githubIssueNumber, 1066);
  assert.equal(
    candidates.some((candidate) => candidate.id === "roadmap-autonomous-task"),
    false,
  );
  assert.ok(triage.evidence.some((line) => line.includes(filePath)));

  const staleLoaded = loadTriageCandidates(dir, NOW + 3 * 86_400_000);
  ...
  assert.deepEqual(loadTriageCandidates(legacyDir, NOW), []);
});
...
test("emitCandidates atomically publishes one current immutable generation", () => {
  ...
  assert.equal(firstCurrent.candidates[0].issueKind, "debt");
  assert.equal(firstCurrent.candidates[0].githubIssueNumber, 1066);
  assert.equal(
    firstCurrent.candidates[0].githubIssueUrl,
    "https://github.com/freed-project/freed/issues/1066",
  );
  ...
  assert.deepEqual(
    ...
    "legacy top-level evidence files are removed after publication",
  );
  assert.deepEqual(

...
|---|---|---|
| GPU backend | Raw WebGPU is selected, Three.js WebGPU remains a reference, and WebGL2 is the deterministic compatibility backend. The shared backend runtime stages hidden candidates while the current canvas stays visible, commits by generation, disposes stale surfaces, retains healthy compatibility state, recovers once, and latches terminal compatibility failure. The shared direct-upload contract defines the eight-float star instances, palette roles, settled square, motion octagon, and decorative motion cap consumed by the worker and raw backend. One shared structural palette contract resolves normalized theme colors, light-surface policy, and the fixed Raw WebGPU uniform block across every backend. | Supply the product factories, scene initialization, and status callbacks to the shared runtime, then verify atomic switch and recovery in the reduced product shell. |

...
- Mac Safari trackpad pinch uses native `gesturestart`, `gesturechange`, and `gestureend` events. Cumulative Safari scale is converted into an incremental ratio on every event, so direction reversal responds immediately through the same midpoint-preserving camera transform used by touch and wheel input.

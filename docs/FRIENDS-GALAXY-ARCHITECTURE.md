...
| Shared runtime contracts | Full-canvas geometry, bounded context anchors, galactic-plane pin coordinates, details requests, keyboard commands, long press, the imperative `fitAll`, `focusNode`, and presentation handle, fixed pointer roster, bounded inertia, scalar settle scheduler, presentation-aware demand-frame predicate, clip-safe camera, depth-correct focus, directional zoom, wheel normalization, midpoint-preserving pinch, fixed diagnostic sample ring, passive long-task monitor, and first-failure backend health latch now live under shared UI source and are consumed directly by the laboratory. | Make the reduced React shell delegate its existing ref, callbacks, input lifecycle, camera, gestures, diagnostics, and recovery to these exact modules, with no translated interaction or observability layer. |

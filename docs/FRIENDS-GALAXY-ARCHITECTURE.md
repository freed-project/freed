...
This exposed two remaining production cutover hazards. `FriendGraph.tsx` currently calls `runAtlasOnMainThread` when its atlas worker is unavailable or exceeds its timeout. That compatibility behavior can turn worker pressure into a frozen Friends interface. Its diagnostic frame history also shifts a JavaScript array after every retained sample. Product integration must replace the compiler fallback with a bounded retry, compatibility state, or retained last-good scene, and use the proven fixed sample ring. It must never compile the full semantic model or maintain diagnostics through per-frame array shifts on the React thread.

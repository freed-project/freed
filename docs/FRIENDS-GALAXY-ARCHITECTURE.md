...
The shared WebGPU shell caps settled Retina rendering at 1.5 device pixels per CSS pixel. Active camera motion temporarily uses 1.0 on compact canvases and 1.25 on wider canvases, then restores settled density after 140 ms. The resize occurs once at motion entry and once at settle. It does not evict stars, rebuild labels, decode avatars, or change semantic detail. Both WebGPU backends report the active render density. This policy follows the existing production engine cap while reducing compact motion fill to less than half of an uncapped 3x iPhone surface.

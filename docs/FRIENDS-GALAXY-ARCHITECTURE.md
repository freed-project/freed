...
The worker-packed semantic and decorative streams are theme-neutral. Each instance carries position, size, brightness, palette role, interaction mix, and alpha scale. Raw WebGPU uploads those streams once. Its star shader resolves eleven active-theme colors from a fixed uniform block, so Scriptorium, Neon, Midas, and Vesper changes do not loop over or reupload resident stars. Live Scriptorium-to-Neon verification kept the resident star upload counter at two while visibly changing the complete field, preserving 30,000 semantic stars, selection, and contextual links.

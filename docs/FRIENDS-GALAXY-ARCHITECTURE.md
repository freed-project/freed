...
The loader admits that worker envelope before creating any renderer. Its constant-time validation checks the scene version, scalar source counts, typed-array classes and expected lengths, scene and pick bounds, adjacency and pick-offset terminal values, packed star stream sizes, rich-metadata cap, and exact receipt counts. It does not scan node ids, star instances, edges, or pick entries. A malformed or mixed-version response closes the one-shot worker and surfaces one explicit startup error instead of passing bad bytes into WebGPU, falling back into a main-thread rebuild, or retaining a partially trusted scene.

...
Outward zoom is constrained by projection math rather than a fixed magic scale. The camera derives a clip-safe floor from viewport height, field of view, semantic minimum depth, and the shared 20,000-unit far distance. It uses 82 percent of that depth for the semantic plane and reserves the remainder for the decorative starfield. An invertible zoom coordinate remains identical to ordinary multiplicative zoom before resistance begins. Inside the boundary region it maps additional outward input through a rational curve that approaches the safe floor continuously. A practical terminal coordinate makes further movement imperceptible before the floor while preserving immediate reverse response. Wheel pinch, native touch pinch, Mac Safari gesture events, and keyboard zoom use this curve. Fit galaxy and resize use the same clip-safe range. Every focal zoom continues to preserve its world point.

...
| Shared runtime contracts | Full-canvas geometry, bounded context anchors, galactic-plane pin coordinates, details requests, keyboard commands, long press, the imperative `fitAll`, `focusNode`, and presentation handle, fixed pointer roster, bounded inertia, scalar settle scheduler, presentation-aware demand-frame predicate, clip-safe camera, depth-correct focus, directional zoom, wheel normalization, midpoint-preserving pinch, fixed diagnostic sample ring, passive long-task monitor, first-failure backend health latch, keyed avatar admission state, bounded avatar image queue, transferable stable-ID and sparse-pick index, allocation-bounded interaction scene state, allocation-free world-to-screen projection, theme-aware label plus avatar billboard compositors, direct-upload star instance plus geometry contracts, the structural theme palette plus fixed Raw WebGPU color block, provider-field geometry plus presentation contracts, source-key incremental activity indexing, and sparse typed activity patch encoding now live under shared UI source and are consumed directly by the laboratory. | Make the reduced React shell delegate its existing ref, callbacks, input lifecycle, camera, gestures, diagnostics, recovery, avatars, stable-ID lookup, sparse interaction lifecycle, presentation admission, overlay projection, billboard construction, palette, provider fields, activity revisions, and worker-to-GPU streams to these exact modules, with no translated interaction or observability layer. |
...
Runtime device loss now follows the same deterministic route. Each WebGPU backend reports its first fatal device error through a one-shot health signal, ignores intentional teardown, and offers a laboratory-only loss control. The shell polls health outside the dirty-render condition, catches synchronous render failures, switches once to the current WebGL2 engine, and preserves the active theme, camera transform, semantic scene, and interaction state. Live forced-loss checks from raw WebGPU and the Three.js reference both retained Neon, close scale, and all 30,000 semantic stars after recovery, exposed the exact loss reason, disabled the destructive control on the fallback, and produced no browser logs. Installed-device loss and resume behavior remain production acceptance gates.

...
The laboratory startup path now compiles its complete 30,000-star semantic fixture and interaction index in a dedicated one-shot worker. Fifteen numeric buffers transfer ownership to the main thread, including positions, visual channels, indexed edges, decorative stars, the stable-id hash table, and adjacency offsets. The UI receives only 192 rich node records plus bounded label and provider metadata. Renderer switching reuses the transferred hash and adjacency tables, so it does not rebuild graph indexes. Worker timeout or failure is explicit and terminal for the laboratory startup. There is no whole-graph main-thread fallback.

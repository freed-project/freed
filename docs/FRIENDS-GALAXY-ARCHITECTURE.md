...
The raw backend pre-records two WebGPU world bundles over shared resources. The base bundle contains the procedural fields, 100,000 decorative stars, and 30,000 semantic stars. The interactive bundle adds one fixed-capacity star overlay after those same draws. Each camera frame writes the shared transform uniform, points one reused render-pass attachment at the current canvas texture, executes exactly one selected bundle, draws only the remaining contextual overlays, and submits through a reused command-buffer container. This removes repeated world pipeline, bind-group, vertex-buffer, and draw setup from the JavaScript gesture path without freezing buffer contents. Theme, field-style, sparse activity, and sparse interaction writes remain visible through the pre-recorded commands. One activity update rewrites only the affected 32-byte base semantic instance and refreshes the bounded overlay once if that identity system is active. A live empty-space pan and same-tier zoom left the buffer-upload counter unchanged, while channel selection, its contextual edge, and a Neon-to-Scriptorium palette change remained visible through the selected bundle.

...
| Shared shell contracts | Full-canvas geometry, bounded context anchors, galactic-plane pin coordinates, details requests, keyboard commands, long press, and the imperative `fitAll`, `focusNode`, and presentation handle now live under shared UI source and are consumed directly by the laboratory. | Make the reduced React shell delegate its existing ref and callbacks to this handle, with no translated interaction layer. |

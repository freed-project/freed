...
- Mac Safari trackpad pinch uses native `gesturestart`, `gesturechange`, and `gestureend` events. Cumulative Safari scale is converted into an incremental ratio on every event, so direction reversal responds immediately through the same midpoint-preserving camera transform used by touch and wheel input. Trackpad inward input uses a constant full-speed log response inside the compressed overview band, then returns to ordinary zoom without discarding the portion of an event that crosses the boundary.
...
| Labels | One instanced billboard glyph batch remains resident during movement, then performs bounded perspective-projected admission after settle. The shared compositor owns theme outline and fill, measurement, packing, UVs, and the stable instance stream while the caller owns candidate ranking. | Connect real product label candidates to the shared compositor, then verify reader and search focus, detail-rail changes, theme switches, and fallback parity in the product route. |
| Avatars | Close detail admits at most 12 desktop or 6 compact avatars. The shared keyed admission state and image queue deduplicate source revisions, bound decoding, reuse results, reject stale completions, cache failures, evict least-recently-used entries, and dispose owned bitmaps. The shared compositor owns image clipping, initials fallback, selected borders, packing, UVs, and the stable instance stream. | Connect real product avatar candidates to the shared admission and compositor modules and prove selected identity priority in Freed Desktop and the PWA. |
| Camera and input | The locked camera has native iPhone touch, Mac trackpad pan and pinch, pointer and touch inertia, midpoint anchoring, reduced-motion cancellation, keyboard navigation, far and near clip-safe scale limits, depth-correct focus, separate full-canvas and bounded interaction rectangles, one accessible graph region, and stable details plus context-action requests. | Reuse these exact primitives in the product engine and complete the maintained menu-focus and search-to-focus workflows. |
...
Raw WebGPU also treats animation and motion as uniform state rather than scene state. The animation toggle now controls shader animation instead of only controlling frame scheduling. When animation is disabled or the camera is moving, one uniform branch skips star-vertex twinkle phase arithmetic and trigonometry and freezes procedural field drift. Raw WebGPU pre-records settled and moving variants of both world bundles. The settled variants draw four-vertex square strips. The moving variants draw regular eight-vertex octagon strips that preserve horizontal and vertical diameter while reducing potential fragment coverage by about 29 percent. Motion selects the prepared bundle without rebuilding geometry or command state. Moving fragments then test squared radius, use multiply-only core and halo falloff, and discard negligible outer-halo alpha before blending. Camera motion exits the procedural field shader after one coarse noise sample, one irregular envelope, and, for stream variants only, one spiral phase. The settled branch restores the complete square footprint and luminous star profile plus seven independent four-octave field samples, two-dimensional domain warping, high-frequency wisps, and both spiral phases. The existing camera-scale float carries motion through its sign, while its absolute value preserves camera-detail fading. This changes no uniform-block size, bind group, GPU instance buffer, resident-star count, label batch, or draw topology at runtime.
...
Raw and Three WebGPU diagnostics now expose independent label-atlas and avatar-atlas build counters. Hover changes neither counter. One selection change, selection clear, or completed pan produces exactly one settled presentation rebuild. The selected close roster may produce one additional avatar rebuild when newly admitted images replace placeholders. Tap selection no longer performs an immediate presentation rebuild followed by a duplicate delayed settle rebuild. These counters make the resident-during-motion contract directly inspectable without treating aggregate buffer uploads as a proxy.
...
Interaction uses the production locked-camera transform. Native touch pan and pinch, pointer pan for mouse and pen, Mac two-finger wheel pan, control-modified wheel pinch, and Safari gesture zoom all update the transform directly. Touch-capable hardware registers non-passive `touchstart`, `touchmove`, `touchend`, and `touchcancel` handlers and suppresses the duplicate touch Pointer Event path. One cached viewport origin is shared across every input path and refreshed only at interaction entry, gesture start, the first wheel event in a burst, and resize. Active touch state lives in one fixed-capacity typed roster with ordered pointer IDs, current positions, and gesture origins. Pan and pinch perform indexed scalar reads and writes without creating a Map iterator or point object. Removing either touch shifts the survivor in place and cancels the former pinch settle deadline, so one-finger continuation cannot trigger a presentation rebuild underneath the remaining contact. Touch cancellation clears the roster before scheduling settle, so a canceled WebKit sequence cannot strand stale contacts. Hover coalescing stores only the latest scalar pair. Safari trackpad zoom stores its viewport origin, previous midpoint, and previous cumulative event scale as scalars. Each `gesturechange` converts that cumulative value into an incremental ratio before applying the directional curve. Reversing inward therefore responds on the first event, even when Safari still reports a cumulative scale below one for the complete gesture. Inside the compressed overview band, trackpad inward input applies a constant three-times log-space response. If one event crosses the band edge, the transfer consumes only the accelerated portion and applies its remaining raw input through ordinary multiplicative zoom. Keyboard zoom and native two-touch pinch continue to use their exact ratios. The handler performs no point-object allocation or viewport geometry read during movement. The laboratory exposes its selected touch and wheel input modes plus viewport geometry-read count in visible diagnostics and inspection state. Pure gesture tests confirm the exact touch-distance scale ratio, moving-midpoint preservation, scale clamping, centered zoom anchoring, immediate Safari reversal, boundary-crossing input preservation, wheel-delta normalization, typed-roster ordering, cancellation reset, and touch handoff. A 390 by 844 browser probe confirmed that a synthetic two-touch expansion changed camera scale by the exact touch-distance ratio while preserving the world point beneath the midpoint.

Pointer and one-finger touch release can continue through one bounded inertial pan controller. Event samples retain only scalar velocity. The demand-driven frame loop consumes one reused result object, applies exponential decay with a capped launch speed, and avoids per-frame square root work unless an input sample exceeds the cap. The controller cancels on new input, resize, backend change, visibility loss, excessive frame delay, or reduced-motion preference. A stopped throw schedules the existing scalar settle deadline once. Mac trackpad pan does not enter the custom controller because macOS already emits its own momentum wheel sequence. Functional desktop checks prove ordinary wheel input changes only camera position, control-modified wheel input changes scale, a drag continues after release, and a tap stops and settles it. A compact native-touch check proves the same post-release continuation. A reduced-motion check proves release adds no movement. All checks retain 30,000 semantic stars and report no browser errors. These are behavioral checks, not timing benchmarks.

Outward zoom is constrained by projection math and fitted galaxy geography rather than a fixed magic scale. The camera derives a clip-safe reserve from viewport height, field of view, semantic minimum depth, and the shared 20,000-unit far distance. It uses 82 percent of that depth for the semantic plane and reserves the remainder for the decorative starfield. A separate viewport-derived envelope places the outer interaction target two percent beyond the padded fitted overview and begins resistance at 1.8 times fitted scale. Outward wheel, native touch, Mac Safari gesture, and keyboard input enter a 0.25-power rational curve whose derivative matches normal zoom at the boundary, then brakes more decisively as it approaches the useful fitted target asymptotically. There is no reachable terminal coordinate or abrupt camera stop. Inward trackpad input inside that band uses a constant three-times log-space response, so the first reverse event creates visible motion even at maximum distance. Input that crosses the resistance boundary keeps its unused portion and completes through ordinary multiplicative zoom. Keyboard and native two-touch pinch retain their exact inward ratios. Safari still uses the ratio between consecutive cumulative gesture values, so direction changes respond before the current gesture ends. Fit galaxy and resize recompute the envelope from current bounds while the deeper clip reserve remains unreachable through ordinary navigation. Every focal zoom continues to preserve its world point.
...
Settled labels and avatars are now selected against the actual perspective-projected viewport. Overview keeps the stable provider and galaxy labels. Middle and close detail scan the resident person indices, retain a bounded prominence-ranked candidate pool, perform collision admission in screen space, and synthesize presentation metadata for visible identities that were not included in the worker's compact metadata sample. The accepted label and avatar bundles stay unchanged during active camera movement. Settle rebuilds only those small presentation bundles. Avatar decode reuse is keyed by the accepted visible identity roster, so a close-range pan admits newly visible images while retaining cache hits shared with the prior viewport. Offscreen global favorites no longer consume the close-detail atlas.

...
This exposed one remaining production cutover hazard. `FriendGraph.tsx` currently calls `runAtlasOnMainThread` when its atlas worker is unavailable or exceeds its timeout. That compatibility behavior can turn worker pressure into a frozen Friends interface. Product integration must replace it with a bounded retry, compatibility state, or retained last-good scene. It must never compile the full semantic model on the React thread.

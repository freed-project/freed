...
Raw WebGPU also treats animation and motion as uniform state rather than scene state. The animation toggle now controls shader animation instead of only controlling frame scheduling. When animation is disabled or the camera is moving, one uniform branch skips star-vertex twinkle trigonometry and freezes procedural field drift. Camera motion now exits the procedural field shader after one coarse noise sample, one irregular envelope, and, for stream variants only, one spiral phase. The settled branch restores seven independent four-octave samples, two-dimensional domain warping, high-frequency wisps, and both spiral phases. The existing camera-scale float carries motion through its sign, while its absolute value preserves camera-detail fading. This changes no uniform-block size, bind group, render bundle, GPU buffer, resident-star count, label batch, or draw topology.

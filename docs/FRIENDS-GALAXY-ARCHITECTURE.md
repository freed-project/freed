...
The worker records the maximum adjacency degree beside its transferable offsets and indices. The scene index uses that scalar to allocate one interaction state, one role map, and one contextual-edge buffer for its lifetime without scanning the complete adjacency table during renderer startup. Each focus change mutates only the buffer's active prefix and count. Every renderer retains its touched and changed index sets. Raw WebGPU copies star and edge components into resident scratch arrays without creating typed-array views. The Three.js reference allocates its maximum edge geometry and sparse attribute-update records at startup, then mutates those buffers and ranges in place. The current WebGL2 fallback restores and reapplies flags through the same retained touched set. Repeated hover and selection no longer create interaction maps, edge arrays, temporary set unions, typed views, or replacement edge geometry in laboratory code.

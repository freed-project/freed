...
| Diagnostics | The laboratory reports renderer, fallback, source scale, star counts, presentation counts, contextual edges, sparse picks, uploads, render density, frame-loop state, and camera safety. One click exports the bounded identity-free schema. | Route the same schema through the product renderer heartbeat, add integrated long-task evidence, and preserve one-click installed export. |
...
The laboratory now exposes one bounded diagnostic export shared by its visible control and inspection API. Schema version 1 contains source and summary counts, renderer and adapter identity, fallback reason, resident and motion counts, draw and bundle shape, presentation counts, upload counters, activity patches, camera limits, viewport dimensions, input mode, recovery state, and fixed-ring frame summaries. It deliberately contains no node IDs, person IDs, account IDs, names, avatar URLs, sample item IDs, or feed payloads. Missing and non-finite backend values normalize to `null`, renderer error text is length bounded, and unavailable long-task evidence remains explicitly `null` until product heartbeat integration supplies it. Clipboard rejection remains local, restores the control, and reports failure through the existing status surface.

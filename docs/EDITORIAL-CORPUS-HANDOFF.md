...
1. Each author works in a separate worktree based on the current `origin/dev` head plus the consolidated reviewed corpus. Do not merge this pull request or another task's branch without Level 3 or higher authorization. Read current repository governance before implementation.

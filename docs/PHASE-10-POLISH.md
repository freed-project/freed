...
Freed has a local nightly improvement planner in `scripts/nightly-self-improve.mjs`. GitHub Issues carrying the `debt` label are its sole canonical backlog. Soaks, bug scans, crash evidence, peer worktrees, prior outcomes, and repository state help rank or update those issues, but they never become executable work by themselves.

The runner selects only open debt issues with healthy evidence, complete authority, satisfied gates, and a matching active execution task. It may batch non-overlapping runtime-neutral work. It admits at most one product behavior change until that change has an installed-build soak outcome. It stops when no eligible issue remains or the run budget expires. It does not scan broad roadmap prose for speculative work.

The checked-in control plane under `automation/` and `scripts/automation-control.mjs` keeps active execution state, append-only events, token-bound leases, conservative lifecycle transitions, and authenticated outcomes. `current-tasks.json` is execution authority only, and every task references its canonical GitHub issue. Provider gates, owner review, strict machine preflight, evidence-derived comparisons, installed identity, one-behavior-slot enforcement, and durable outcome transactions remain unchanged. `npm run stability:status` now reduces repository identity, observed runtime identity, control and outcome health, behavior-slot ownership, static actor parity, soak maturity, and the content-addressed artifact index into one human summary and one versioned JSON model. Its stable digest ignores observation time and machine paths, while its source classifiers distinguish healthy, unavailable, stale, malformed, and unsupported evidence without recovering or mutating control state.

AubTown factory workers now use task-scoped execution claims inside that same
atomic task manifest. One trusted nightly coordinator lease may acquire,
heartbeat, checkpoint-transfer, or release a claim, while workers receive only
the exact short-lived task admission. The pilot admits runtime-neutral,
provider-forbidden, nonbehavioral work with a draft pull request ceiling.
Claims bind one task revision, issue, host, worker, branch, worktree, conflict
set, custody epoch, base head, account, driver, and target. Exact operation
retries recover after response loss, changed replays fail closed, transfers
advance one checkpoint-backed epoch, and stale heartbeats or releases cannot
displace current custody. Claims remain a projection of execution authority,
not a second queue or scheduler. This control-plane extension adds no provider
traffic and does not change Phase 10's `upcoming` roadmap status.

One reviewed opaque host ID now designates the sole `primary-automation-host`. The designated Mac proves that assignment with a root-owned local profile. Only that host may activate or acquire the nightly executor. A single Codex heartbeat owns host maintenance, current callable runner-model enforcement, and conservative external archival of confirmed no-op automation tasks. Failures, authority blocks, actionable findings, changes, and user decisions remain visible. Host-local automation evidence is not synchronized or mistaken for distributed consensus.

Every general automation actor now requires lease acquisition through a trusted host launcher with no persistent general actor credential in Keychain or local automation state. The owner-run bootstrap helper builds deterministic native binaries with linker-generated ad hoc signatures and requires no developer signing identity. It installs an actor-specific root-owned launcher plus one content-addressed root-owned runtime that pins Node, both control entries, both control libraries, the kernel guard and outcome repair contracts, and the lease archive helper. The launcher creates a fresh operation ID and 32 random bytes for each acquisition, sends the raw lease token only through a one-use file descriptor channel, and binds its digest to the native process chain and random challenge. Public general actor acquisition fails closed. General actor leases have a 30 minute absolute lifetime, and exact retry recovers a committed result after response loss. Verification validates every public pin and proves a live nonmutating channel attestation. Host acceptance proves acquire, heartbeat, release, and stable launcher identity for all five actors before activation. The normal host has no Security framework dependency. A separate migration-only native provisioner deletes validated schema 1 Keychain items and digest records with interaction disabled before schema 4 replacement, tolerates every item and record presence combination, and is safe to retry. Rotation is removed. Saved actor reconciliation validates the schedule, current callable model and supported reasoning effort, canonical Freed target, exact working-directory scope, execution environment, launcher and runtime digests, and live channel attestation. Missing actors remain paused. Active actors fail closed until real-host verification and lifecycle acceptance prove the complete handoff. The launchers prove the selected role, pinned runtime, process chain, operation identity, token digest, and one-use challenge, but they cannot authenticate which process under the same macOS user invoked that role. Cross-role isolation remains cooperative. Stored task authority, provider approvals, the global behavior slot, owner governance, publisher isolation, and GitHub review remain enforced. This bootstrap excludes the owner and PR publisher identities and grants no provider authority. Normal pull request publication remains available through the governed helper and the caller's existing GitHub authentication. Hosts that need stronger unattended publication hardening may optionally install the separate native signed broker. It clears inherited process state, validates its root-owned trust configuration and pinned tools, and uses an Ed25519 Keychain key to issue one short-lived target-scoped capability and publisher lease. The repository does not install that host profile. Missing broker provisioning does not block normal publication, and a partial broker handoff fails closed. This optional profile is cooperative hardening, not an operating-system sandbox against arbitrary same-user code. The owner governance identity remains protected by its separate expiring one-time bootstrap.
...
A stranded event-history predecessor witness now has its own owner-governed repair path. The read-only planner admits one unique exact byte predecessor, one canonical one-event semantic successor, healthy lease, task, manifest, and outcome histories, and an unchanged task-manifest generation. Apply writes a durable task-bound owner authorization before exact-generation retirement, then appends one reserved audit event. The same plan resumes after process loss at the authorization, retirement, or audit boundary without requiring an expired confirmation or duplicating evidence. Changed, settled, ambiguous, or foreign generations fail closed. This runtime-neutral repair adds no provider contact and does not change Phase 10's `upcoming` roadmap status.
...
Library Core release activation now uses the same private current-task confirmation contract when the owner approves the exact release in plain English in the active task. The confirmation binds the reviewed release artifact, product commit, manifest and inspection digests, transition set, tag, channel, approval time, and expiry. The release artifact retains only the deterministic task reference and confirmation digest. GitHub HTML approval comments are no longer required, while legacy comment references remain readable for existing releases.

Rejected legacy outcome history now has a dedicated owner-governed repair contract instead of a manual file-edit escape hatch. One exact owner intent binds the stable canonical task ID, source identity, immutable event-history prefix, physical line count, expected trusted and rejected counts, replacement identity, raw archive, per-line decisions, and completion receipt. Task state and revision remain informational because lifecycle state does not authorize history repair. The owner lease may append a suffix event, but the planned prefix cannot change and full-history reclassification under the shared outcome writer lock must preserve every decision and retained or rejected byte. Authority is rechecked after lock acquisition and immediately before mutation and audit. The reserved audit event has no standalone append surface. Its transaction-bound guard authenticates every prepared archive, reclassifies the immutable source against current event history, verifies the canonical replacement, and permits return only after the exact event and `audited` phase agree. Repair never re-signs legacy lines and recovers idempotently through prepared, replaced, audited, and verified complete phases. Continuous health revalidates completed archives, receipts, and audit events, so corruption closes the behavior slot again. A separate one-time owner-confirmed cutover installs old-compatible PID 1 sentinels and permanent kernel-lock inodes before any new control mutation. Its global receipt activates the new protocol only after every sentinel, the permanent bootstrap lock, and every archive is durable, so old binaries remain excluded and process loss releases only the kernel lock. One aggregate plan limit is shared by planning and every later verifier. A separately owner-confirmed pre-marker supersede operation preserves the abandoned transaction and archives, restores exact planned legacy lock bytes, and permits a fresh plan after protected source drift. Once a permanent writer or guard marker exists, only same-plan resume is allowed. This hardening does not change Phase 10's `upcoming` roadmap status, so no `www` roadmap state change is required.
...
| 10.31 | Stranded event-history witness repair    | Medium     | ✓ Complete (read-only exact-prefix planning, durable owner authorization before retirement, reserved audit event, idempotent crash recovery, and fail-closed cross-history checks)                                                       |
...
- [x] Library Core release activation accepts the owner's explicit in-band English approval through one exact private current-task confirmation, without requiring a GitHub HTML comment
- [x] Local nightly improvement runner selects eligible GitHub debt issues through strict evidence, authority, provider, and one-behavior-slot gates. The atomic active task manifest references the canonical issue, while leases, outcomes, soaks, canary bundles, and stability artifacts remain operational evidence rather than a second backlog.

...
- [x] The dormant replacement-replication foundation now closes one package-internal immutable object-key contract for epoch certificates, actor enrollment, operation segments, checkpoint manifests and pages, scrubbed Desktop checkpoints, search generations, PWA intents and results, blobs, and backup manifests. Its control-pointer parser permits exactly one non-expiring writer epoch and one active Google Drive or future Dropbox transport, snapshots closed data-only records, and rejects live SQLite, WAL, SHM, rollback-journal, traversal, heartbeat, expiry, dual-authority, and unknown transport inputs. It performs no cloud I/O, changes no Google request cadence, grants no PWA canonical authority, and leaves Automerge as the replication authority

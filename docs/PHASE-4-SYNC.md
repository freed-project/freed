...
> **Current Library Core cutover:** Freed Desktop imports the retained Automerge library once into native SQLite schema v2, then initializes, queries, mutates, searches, exports, diagnoses, and creates 24 retained daily backups without starting the Automerge worker or retaining the item corpus in React. The old LAN relay and Automerge cloud loops are disabled. The retained Automerge bytes are a pre-import rollback source only. The replacement Google Drive path streams one exact SQLite revision into bounded logical checkpoint pages, verifies immutable uploads, advances one provisioned control file through its exact ETag, imports a newer remote checkpoint back into SQLite, and transfers the non-expiring writer epoch through one confirmed compare-and-swap. The PWA can now discover the sole published control, authenticate and import its immutable checkpoint into IndexedDB, hydrate a bounded initial product window without starting its Automerge worker, and page the complete ordinary feed directly from the selected IndexedDB generation. Desktop and PWA Google Drive activation remains default-off pending its separate provider review. PWA intent publication, result receipts, continuous immutable generation refresh, and activation evidence remain in progress.

...
> **Current Library Core cutover:** Freed Desktop imports the retained Automerge library once into native SQLite schema v2, then initializes, queries, mutates, searches, exports, diagnoses, and creates 24 retained daily backups without starting the Automerge worker or retaining the item corpus in React. The old LAN relay and Automerge cloud loops are disabled. The retained Automerge bytes are a pre-import rollback source only. The replacement Google Drive path streams one exact SQLite revision into bounded logical checkpoint pages, verifies immutable uploads, advances one provisioned control file through its exact ETag, imports a newer remote checkpoint back into SQLite, and transfers the non-expiring writer epoch through one confirmed compare-and-swap backed by a native signed epoch certificate. The PWA can discover the sole published control, authenticate and import its immutable checkpoint into IndexedDB, hydrate a bounded initial product window without starting its Automerge worker, page the complete ordinary feed directly from the selected IndexedDB generation, scan the complete selected generation in bounded pages for full-library search and shared aggregate surfaces, retain a nonextractable per-Library actor key, publish a proof-only enrollment request, install the Desktop-countersigned enrollment, and enqueue signed read-state intents without writing Automerge. Desktop and PWA Google Drive activation remains default-off pending its separate provider review. Desktop intent acceptance, result receipts, continuous immutable generation refresh, and activation evidence remain in progress.
...
| 4.122 | Freed Desktop local SQLite cutover with one-time legacy import, SQLite-only normal startup and mutation, bounded reader scans, final renderer corpus eviction, disabled Automerge relay and cloud loops, and verified 24-generation SQLite backup and restore | ✓ | High |
...
- [x] An explicit PWA Library Core Google Drive sync now discovers each bounded local actor outbox with unpublished signed transactions, provisions that actor's empty mutable head by private library and actor digests, uploads and verifies each immutable intent segment, advances only the exact actor head with its current Drive ETag, and records publication locally only after canonical head readback matches. A `412` returns the actual current head, exact response-loss replay recovers idempotently, actor ranges remain bounded, and no intent is represented as accepted or provider-complete. The path remains behind the existing default-off PWA Library Core activation key, adds no provider action, and does not change current Automerge authority.
...
- [x] Provider settings now scan exact source-fenced 64-row pages from the authenticated selected SQLite generation instead of mounting the full compatibility corpus. Facebook group repair carries compact group identities across pages. Facebook and Instagram media backup stages compact local candidate pages, closes the final source fence, then streams one page at a time into the existing archive path. YouTube synchronization retains only deduplicated visible saved-video identities. The normal path has no compatibility lease. A stale, canceled, or unavailable native reader fails closed before provider work or success, and `freed.libraryCore.providerSettingsReaderV1.disabled=1` explicitly restores the previous compatibility projection. Existing controls remain user-triggered, YouTube retains its 25-action cap, and no automatic request, cadence, navigation, cookie, header, or retry behavior changes. Deterministic SQLite global-ID order replaces renderer recommendation order for these deliberate batches under the recorded provider behavior approval. Automerge remains authority, and development performs no live provider traffic.
...
- [x] SearchJump now reads exact tags and archive facets, simple-scope unread and archivable aggregates, complex-scope counts through source-fenced 64-row pages, and one selected item detail without mounting the Desktop corpus. Cancellation stops the scanner and source movement fences detail results. The device-local `freed.libraryCore.searchJumpReaderV1.disabled=1` switch restores compatibility. Executing a no-query bulk mutation still acquires and immediately releases the shared compatibility projection until the native frozen-predicate bulk contract replaces renderer ID enumeration. Automerge remains authority.
...
- [x] The active PWA Library Core runtime now supplies the shared bounded Library scanner and item-detail reader directly from the authenticated selected IndexedDB generation. It visits at most 32 stored rows per IndexedDB page, returns only feed items to each callback, honors early cancellation, and lets full-library search, facets, command surfaces, and locally preserved reader content operate beyond the initial 512-card renderer window without starting Automerge. The existing search path retains at most 100 result cards and releases its disposable term index after the query clears. This adds no cloud or provider request and remains behind the existing default-off PWA Library Core activation key.

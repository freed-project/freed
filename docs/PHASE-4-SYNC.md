...
| 4.42 | Native canonical transaction reconstruction and Ed25519 verification feeding the sealed authoritative SQLite journal input | ✓ | High |
...
- [x] The dormant native operation verifier parses the exact bounded duplicate-preserving canonical envelope bytes inside Rust, reconstructs every payload, member, transaction, actor-chain, signing-body, and envelope digest, requires one complete dense transaction against the exact enrolled actor tip, and verifies every Ed25519 signature before it can construct the journal's private commit input. The immediate authoritative commit then rechecks the actor tip and every exact causal tip. Neither the shared verifier nor renderer-supplied derived fields can grant commit authority, and no command, opener, provider behavior, or runtime activation exists

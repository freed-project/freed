...
- [x] The dormant SQLite stage copies every verified Automerge change, operation, dependency, successor, scalar descriptor, and exact payload through private scratch transactions. Unsigned counters remain fixed-width sortable bytes, payloads stream into preallocated SQLite blobs, and each complete source and run receipt commits with its rows only after post-consumption verification succeeds. Change and operation receipts in one stage must bind the same exact source. The stage compares its exact schema catalog before reuse, returns an original receipt after response loss only while receipted counts remain complete, and rejects changed sources, mixed sources, changed summaries, missing or unreceipted rows, schema drift, tampered companion spools, and injected mid-stage failure without committing partial state. It does not reconstruct objects, populate the published shadow generation, register a command, contact a provider, change the active writer, or activate Library Core

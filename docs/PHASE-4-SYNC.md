...
>
> Local relay, Google Drive cloud sync, desktop local snapshot rotation, "Sync Now" button, "Last synced" indicator, proxied Google token exchange for Freed Desktop with a built-in production proxy default, durable Google OAuth refresh, recoverable Google Contacts token-refresh failures, a production callback relay for dev and preview PWA Google OAuth, appDataFolder Drive polling, cloud sync health diagnostics, visible Drive transfer diagnostics in Settings, manual Drive sync from Desktop and PWA Settings, cloud sync activity timelines, global background activity visibility for Desktop cloud work, initial Drive download auth-refresh recovery, merged-upload local convergence, destructive Automerge merge blocking, pinned explicit local wins and cloud wins recovery actions, PWA local-change cloud uploads, PWA document-init-gated cloud startup, runtime-gated cloud upload waits, mobile-safe Drive upload bodies, the multi-Desktop request warning, the no-cloud-sync launch banner, revision-fenced IndexedDB v2 persistence, stale worker retirement, a production-located compatibility bridge that writes exact Automerge revisions into crash-safe immutable SQLite shadow generations, a dormant receipt-bound external-memory path through graph staging, FeedItem reconstruction, lossless row projection, and bounded shadow-generation population, a closed package-internal protocol for the existing default bounded SQLite feed page, and a registered but unassigned Freed Desktop reader for that protocol are implemented. Automerge remains the sole authority and renderer read source. The compatibility bridge still decodes the complete Automerge document, while the external path has no production migration caller. The dormant Freed Desktop reader pins at most two authenticated immutable generations for at most 60 seconds, caps each SQLite cache at 2 MiB, releases exhausted or cancelled sessions, and returns source-bound canonical cursors through the closed protocol. No product surface calls it. Product filters, recommendation-order equivalence, a product read cutover, and PWA parity remain blocked. These paths therefore do not satisfy the external-memory Gate C migration or deliver the target memory reduction by themselves. Dropbox remains behind a coming-soon gate while its provider work is finished. iCloud is the remaining core document-sync item. Large offline media uses a separate future transport plan.
...
- [x] The dormant Freed Desktop `feed_page_v1` runtime resolves only an already-created private shadow root, authenticates and physically pins the selected immutable generation, and serves the exact closed request and response without assigning a product caller. One shared pool admits at most two sessions, fixes each SQLite page cache at 2 MiB, expires snapshots after 60 seconds, and releases them on exact cancellation or cursor exhaustion. Canonical cursor bytes have one shared TypeScript and Rust vector. Cursor generation, transition, projection revision, order, operation identity, row count, and complete serialized response bounds fail closed. PWA adapters, product filters and recommendation ordering, the renderer cache, and product read cutover remain blocked, so Automerge stays authoritative and no provider behavior changes
- [x] The dormant PWA `feed_page_v1` runtime stores compact feed rows individually in IndexedDB and registers dedicated bounded request and response kinds in the existing Automerge worker transport. Bounded pages of at most 128 rows stage in exact keyset order with a SHA-256 replay receipt, unique generation-plus-entity identity, contiguous batch indexes, exact source identity, and durable row counts. Final selection is one transaction and cannot expose an incomplete generation; restart resumes staged work, exact finalization replay is idempotent, and later selections retain at most one rollback generation. Reads share the closed TypeScript protocol, admit at most two 60-second logical sessions, reject expired or replaced cursors, bind exact cancellation identity, and release exhausted sessions. No product surface calls the reader, no authoritative operation is written, Automerge remains the PWA writer and renderer source, and provider behavior is unchanged

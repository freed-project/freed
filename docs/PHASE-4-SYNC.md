...
- [x] The current renderer and future bounded Library Core adapters share one allocation-free feed-filter predicate and one canonical normalized filter record. Exact strings remain byte-for-byte significant, set-like tags and signals are deduplicated and binary-sorted for stable cursor binding, and the contract pins hidden, archived, RSS identity, platform, author, feed URL, post/story, saved, tag, and content-signal behavior. The active renderer now normalizes once per browse pass instead of once per item. Native and IndexedDB filter pushdown, recommendation ordering, bounded archived generations, renderer-cache eviction, and product caller proof remain pending, so this contract does not activate a Library Core reader

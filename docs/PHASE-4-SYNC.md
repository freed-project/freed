...
| 4.96 | Dedicated native browse-generation registry with exact logical query and physical file identity, replay-safe registration, atomic selection, one rollback generation, and no product caller | ✓ | Medium |
...
- [x] The dormant native browse registry accepts only a sealed generation whose logical source, filter, ranking clock, recommendation-order version, row count, physical SHA-256, byte length, and file name match a fresh read-only inspection. Registration is idempotent only for the complete identical receipt and rejects changed logical or physical identity. One immediate SQLite transaction selects the current generation, retains exactly one rollback generation, records a request-digest-bound transition receipt, rejects stale expected state, and replays a lost response without advancing twice. The registry has a distinct schema identity and exact catalog check, never blesses an existing foreign or empty file, and exposes no Tauri command, product reader, provider action, writer change, or Library Core activation

...
- [x] The dormant authoritative SQLite kernel uses WAL with `synchronous=FULL`, a closed versioned STRICT schema, private verified-input types, exact actor-enrollment and transaction retry receipts, immutable enrollment-certificate retention, enrollment and operation replication outboxes, an actor-tip compare-and-swap, causal-tip existence checks, the immutable operation journal, monotone earliest-read materialization, and projection revision in immediate transactions. Fault injection proves that late enrollment-outbox or operation-outbox failure leaves no partial authority, journal, materialized field, receipt, revision, or actor-tip movement. The module has no command registration or production opener, and native canonical plus signature verification must construct its sealed input before activation

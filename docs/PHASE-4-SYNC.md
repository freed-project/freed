...
- [x] The dormant Freed Desktop browse materializer binds one worker session to the exact durable document ID, sorted-head digest and count, storage generation, save revision, normalized filter, safe ranking clock, and recommendation-order version. It counts through one iterator, retains a second iterator plus one replayable page instead of a corpus-sized ID or row array, validates every shared closed feed-card DTO, and emits at most 128 rows per page. The main-thread adapter replays pages from zero against native receipts after restart, recovers committed append and finalization responses without guessing, and quiesces before factory reset. No selected-generation registry, native browse reader, renderer caller, provider action, or authority change exists. Automerge remains the sole product reader and writer

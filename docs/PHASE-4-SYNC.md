...
| 4.73 | Atomic scratch SQLite operation staging with incremental payload blobs, exact schema verification, and response-loss receipts | ✓ | Medium |
...
- [x] The dormant operation stage copies every verified Automerge operation, successor, scalar descriptor, and exact payload into one private scratch SQLite transaction. Unsigned counters remain fixed-width sortable bytes, payloads stream into preallocated SQLite blobs, and the complete source and run receipt commits with the rows only after post-consumption verification succeeds. The stage compares its exact schema catalog before reuse, returns the original receipt after response loss only while receipted counts remain complete, and rejects changed sources, changed summaries, missing or unreceipted rows, schema drift, tampered companion spools, and injected mid-stage failure without committing partial state. It does not reconstruct objects, populate the published shadow generation, register a command, contact a provider, change the active writer, or activate Library Core

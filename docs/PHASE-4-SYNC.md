...
> **Current Desktop cutover:** Freed Desktop imports the retained Automerge library once into native SQLite schema v2, then initializes, queries, mutates, searches, exports, diagnoses, and creates 24 retained daily backups without starting the Automerge worker or retaining the item corpus in React. The old LAN relay and Automerge cloud loops are disabled. The retained Automerge bytes are a pre-import rollback source only. The first replacement Google Drive path now streams one exact SQLite revision into bounded logical checkpoint pages, verifies immutable uploads, and advances one provisioned control file through its exact ETag. Startup and Sync Now are wired to this path behind the default-off Drive activation gate. Writer reassignment, remote checkpoint import, the PWA IndexedDB client, and activation evidence remain in progress.

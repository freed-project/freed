...
The trigger helper is not a traffic generator. If the renderer is rebuilt after the provider run already finished, stop and inspect the health log or wait for the provider's cooldown instead of queueing a new request. The native watcher must only replay the same request when the renderer was rebuilt before the sync trigger finished. Runtime deferrals should use provider-safe spacing: Facebook can retry sooner, Instagram needs the 10 minute interval, and LinkedIn needs the longer LinkedIn interval. A locked Mac is different because no provider scrape has started, but locked retries should still be sparse so an unattended run does not churn the renderer. The helper defaults locked-machine retries to 10 minutes. Use `FREED_DEV_SYNC_TRIGGER_TIMEOUT_MS=21600000 node scripts/dev-sync-trigger.mjs <provider>` when a six-hour soak should keep waiting for unlock.

...
| 6.46 | Move PWA actor enrollment, normalized intent publication, normalized result import, and exact cloud receipts from the retired portable store onto the existing closed OPFS SQLite transport transactions. The normalized intent and result transactions now cross the closed worker boundary. Enrollment and Drive orchestration remain open. | High       | 🚧 In Progress                                                                                   |

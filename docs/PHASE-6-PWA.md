...
- [x] Persist the Library, query indexes, search, intent outbox, result receipts,
  ...
  - [x] Apply one closed authority-signed Primary result through the same
        ...
        field identity set, and settles the immutable result, intent, and result
        cursor in one immediate transaction. Accepted results then enter the
        version 2 operation importer with their exact locally stored
        actor-signed members. The result handler never materializes canonical
        rows or advances the source revision. A later result is retained
        without skipping intervening authoritative revisions. Exact retries
        return the durable receipt and resume operation application. Changed
        bytes, result-chain gaps, incomplete projections, stale authority, and
        a late cursor fault cannot partially settle the result.
  - [x] Materialize accepted `friend_replace` results atomically in browser
        SQLite with native Rust parity through the sole version 2 operation
        transaction. The complete desired Person, Person tags, linked Account
        set, Account follow roles, detached social Accounts, replaced contact
        Account, Person invalidation, and Account reset invalidation commit with
        the journal and applied-result proof. Fractional binary64 wrappers are
        decoded only after canonical signature verification and before strict
        SQLite REAL columns are written.
  ...
  - [x] Import one closed normalized v2 result transport segment through OPFS
        ...
        one immediate transaction. Result settlement, logical cursor
        advancement, immutable-object receipt, and transport-head advancement
        commit or roll back together. Accepted results then resume the sole
        operation importer. Its canonical transaction removes the optimistic
        overlay only after journaling and materialization succeed. Exact
        segment retry completes a previously failed operation application,
        while changed bytes or object identity fail closed.
  - [x] Import normalized operation protocol v2 pages through the same closed
        SQLite worker boundary. Accepted Primary results and actor-signed
        members may arrive on separate pages or ahead of the current revision,
        but no product row changes until the exact next complete transaction
        verifies. Transaction rows, operations, causal tips, typed projections,
        receipts, invalidations, actor tip, source revision, and the applied
        result proof commit atomically. A follower creates no replication
        outbox. Exact retries are idempotent, changed records fail closed, and a
        late proof fault leaves the complete staged transaction available for
        retry without partial materialization.
...
- [x] Serve every product surface through bounded named SQLite queries without
  ...
  - [x] `optimistic_fields_v1` returns only current-actor fields for at most 64
        visible FeedItem identities. Views merge the shared sparse response and
        drain local changes without incrementing canonical SQLite revision.
...
| 6.41 | Import normalized v2 result transport segments through PWA OPFS SQLite, including semantic digest and signature verification, atomic result-chain and transport settlement, exact retry receipts, and resumption through the sole normalized operation materializer. Keep the sparse overlay until that operation transaction commits, and prove exact replay after a late operation-proof fault                                                                                                                                                                                                                                                                                                                                                                                               | High       | ✓ Complete                                                                                       |
...
| 6.46 | Move PWA actor enrollment, normalized intent publication, normalized result import, and exact cloud receipts from the retired portable store onto the existing closed OPFS SQLite transport transactions. The closed worker boundary and transport-neutral coordinator cover enrollment, bounded intent publication, response-loss repair, and atomic result reconciliation. The PWA binds that coordinator to the approved Google Drive adapters after normalized checkpoint import without changing adapter request behavior.                                                                                                                                                                                                                                                               | High       | ✓ Complete                                                                                       |
...
| 6.83 | Install the same normalized device-local Google Contacts generation tables in OPFS SQLite as the native core. The closed worker mutation boundary begins, appends, matches, activates, updates status, and dismisses suggestions through replay-safe bounded batches. The OPFS worker runs the same byte-capped status, building-generation match, active suggestion, and unmatched-contact keyset queries as native Rust. Rust and TypeScript consume the same generated bounds and digest identity. The PWA product uses this boundary for Settings, Friends, review paging, and background sync. React retains one status row and visible review pages. Contact rows remain outside checkpoints and replication. One startup import consumes and deletes the historical localStorage ledger | High       | ✓ Complete                                                                                       |
| 6.84 | Remove complete FeedItem, RSS Feed, Person, Account, Friend, and per-feed count collections from the PWA Zustand contract. Invalidate bounded OPFS SQLite query windows after typed mutations, run sample cleanup through shared source-fenced maintenance scans, preserve real Accounts linked to sample Persons, and retain no optimistic whole-corpus projection                                                                                                                                                                                                                                                                                                                                                                                                                            | High       | ✓ Complete                                                                                       |
...
| 6.97 | Export normalized checkpoints directly from OPFS SQLite through the closed worker protocol. Pin every bounded page to one typed descriptor, preserve native causal-frontier digest parity, reject revision drift and unresolved local intents, enforce exact response and record ceilings, reassemble maximum legal content chunks, and expose one local descriptor plus header proof through bounded Sync diagnostics without a shell or database-file transport                                                                                                                                                                                                                                                                                                                              | High       | ✓ Complete                                                                                       |
...
| 6.102 | Delete the PWA performance gate that benchmarked an uncalled full-corpus identity graph implementation. Friends Galaxy correctness and SQLite paging stay covered by the shipping product-worker and bounded-source suites, while raw timing evidence remains in nightly and installed-build performance lanes | Medium     | ✓ Complete                                                                                       |
| 6.103 | Delete shared renderer fallbacks that could re-rank a Saved FeedItem collection or collect tags by scanning FeedItems. PWA Saved ordering and tag navigation now have only their bounded OPFS SQLite query sources | Medium     | ✓ Complete                                                                                       |
| 6.104 | Delete dormant PWA tests and shared helpers that rebuilt identity, map, or graph activity from complete Person, Account, and FeedItem collections. OPFS SQLite supplies compact graph rows, map candidates, facets, and activity counts directly | High       | ✓ Complete                                                                                       |
...
| 6.117 | Add a nonblocking PWA OPFS corpus hardening lane that streams deterministic provider-neutral FeedItems in bounded 128-row transactions, records browser memory and storage evidence at 25,000 and 100,000 items, and proves feed, facet, and search responses stay inside their row and scan limits. Run the 100,000-item envelope in Chromium and the 25,000-item contract milestone in WebKit. Unsupported memory measurement remains explicit instead of fabricating evidence | High       | ✓ Complete                                                                                       |
...
| 6.122 | Report sample Library clearing through one persistent toast driven by successfully committed bounded removal batches, keep progress monotonic through final settlement, disable repeat maintenance actions while clearing, and restore sample population only after SQLite state invalidation completes | Medium     | ✓ Complete                                                                                       |
...
- [x] Nightly PWA corpus hardening exercises real OPFS SQLite at 25,000 and 100,000 deterministic provider-neutral FeedItems in Chromium plus the 25,000-item contract milestone in WebKit, records browser memory and storage measurements, and proves bounded feed, facet, and search behavior without adding a pull request gate

...
- [ ] Runtime state now hydrates one bounded visible OPFS SQLite feed window, synchronized preferences, exact trigger-maintained navigation aggregates, and the selected checkpoint receipt without a shell. Move the remaining feed, Person, and Account catalog surfaces plus sample-data maintenance inputs to dedicated bounded SQLite pages before declaring the runtime-state cutover complete.

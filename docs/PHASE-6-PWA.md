...
- **People mutations:** Person add, bounded batch add, synchronized profile updates, bounded reach-out history, and atomic removal with linked-account cleanup enter the signed epoch-scoped intent outbox and update the selected IndexedDB Library shell immediately. Device-local graph coordinates stay outside canonical payloads, and reach-out history retains its 20-entry cap without waking Automerge.

...
- `@freed/sync` owns a transport-neutral Primary coordinator with injected
  ...
  adapter.
- `@freed/sync` also owns provider-neutral bounded Primary follower
...
- `@freed/library-service` binds those coordinators to the generated native
  ...
  SQLite, then returns the existing bounded canonical page contract.
...
| 11.6  | In Progress | Open final normalized SQLite behind the descriptor-bound sidecar and provide generated bounded checkpoint, pinned export, registered query, Primary signing, canonical commit, authority-signed follower enrollment, follower-intent admission, actor state, and result export commands. Provider-neutral bounded enrollment, intent ingestion, and result publication orchestration now binds to the native command channel. Sequence-bound result export resolves its exact predecessor inside SQLite. Complete the installed transport binding next.                           |

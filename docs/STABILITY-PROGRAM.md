...
- **Provider-visible lane.** Anything that changes WebView loads, provider navigation, request frequency, cookies, headers, or extractor scripts uses two owner gates. Gate 1 approves the named observable behavior, fingerprinting risk, and lower-profile alternative before implementation. Gate 2 happens after a candidate diff is committed. Its external JSON record cannot be future-dated, lasts at most seven days, exactly matches the provider-visible path set, maps every path to its provider scope, and binds the full committed binary diff hash. The owner replaces the preliminary task reference with that packet digest through the private one-time bootstrap flow. Publish with `--provider-risk-approval-file <approval.json>` only while the branch remains clean. Self-attested files fail closed. The PR stays draft until CODEOWNER review and a separate owner-authorized ready transition.
...
| Writer ownership | Private actor credentials authenticate normal lease acquisition. The PR publisher instead consumes one broker-signed capability and never exports its signing key. Token-bound TTL leases then prevent duplicate mutating agents. Active leases cannot be stolen; expired takeover is recorded. |

...
- **Provider-visible lane.** Anything that changes WebView loads, provider navigation, request frequency, cookies, headers, or extractor scripts uses two owner gates. Gate 1 requires explicit approval of the named observable behavior, fingerprinting risk, and lower-profile alternative before implementation. General permission to proceed with the program does not count. After implementation, the branch may publish as a draft. Draft publication does not authorize provider traffic. The helper posts a GitHub review comment bound to the provider-visible path set and provider-only binary diff. Gate 2 is a CODEOWNER thumbs-up reaction on that comment. Provider-visible edits create a new fingerprint and require a new reaction. Unrelated branch edits preserve the existing approval. A signed `control-task` record remains available for machine-verifiable unattended publication and binds the same provider-only fingerprint.

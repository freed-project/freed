...
- **Provider-visible lane.** Anything that changes WebView loads, provider navigation, request frequency, cookies, headers, or extractor scripts uses two owner gates. Gate 1 requires explicit approval of the named observable behavior, fingerprinting risk, and lower-profile alternative before implementation. General permission to proceed with the program does not count. After implementation, record Gate 1 in a healthy `provider-risk-review` stability artifact and publish the branch as a draft with `--provider-risk-review-artifact`. Draft publication does not authorize provider traffic. The helper posts a GitHub review comment bound to the artifact, provider-visible path set, and provider-only binary diff. Gate 2 is a CODEOWNER thumbs-up reaction on that exact, unedited comment. Provider-visible edits or changed Gate 1 context create a new review request and require a new reaction. Unrelated branch edits preserve the existing approval. A signed `control-task` record remains available for machine-verifiable unattended publication and binds the same provider-only fingerprint.

...
```typescript
// Platform types
export type Platform = 'x' | 'rss' | 'youtube' | 'reddit' | 'mastodon' 
  | 'github' | 'facebook' | 'instagram' | 'linkedin' | 'saved';
...
export function updateFeedItem(doc: FreedDoc, globalId: string, updates: Partial<FeedItem>): void;

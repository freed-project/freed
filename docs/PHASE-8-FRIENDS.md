...
> **Status:** In Progress, the canonical identity model uses `Person` plus attached `Account` records, the Friends workspace defaults to `All content`, and the shipping Friends route has been cut over to the next-generation GPU-resident Friends Galaxy. Raw WebGPU is preferred, WebGL2 is the retained compatibility backend, every semantic identity remains resident during movement, the locked camera supports native iPhone touch and Mac trackpad navigation with inertia, Nebula provider fields and billboard labels follow every active theme, linking and pinning remain context-menu-only, and mobile Details suspends the exact resident scene instead of unmounting it. The broader Friends phase still includes unfinished native contacts, overlap, Mozi, and document-worker activity-index work.
...
| 8.161 | Move graph and overview activity summaries into the document-worker index so entering Friends does not materialize or scan the full item library | High | Planned |
...
- [ ] Friends entry consumes document-worker summaries and selected-identity item queries without scanning the full item library in React

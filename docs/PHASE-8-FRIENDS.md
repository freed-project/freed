...
| 8.167 | Match the Friends detail rail's vertical card insets to the primary navigation rail, keep both resize rails full-height and centered in equal gutters, clamp wheel and pinch zoom at the exact per-canvas Fit All scale, and move graph diagnostics into the Friends sidebar menu | Medium     | Done        |
...
- [x] Outward zoom approaches the exact Fit galaxy scale for the current canvas while retaining a deeper far-plane reserve, and wheel, Safari, touch, and keyboard input restore native inward speed on the first reverse event

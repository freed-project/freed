...
The locked camera now derives a deep clip reserve from viewport height, perspective field of view, semantic scene depth, and the shared 20,000-unit far clip distance. It separately derives an outer interaction target from the current fitted galaxy bounds. Wheel pinch, Safari gesture events, native touch pinch, and keyboard zoom decelerate asymptotically toward that fitted target without reaching an abrupt stop or the deeper clip reserve. The directional transfer curve begins outward resistance at 1.8 times fitted scale and brakes more firmly as it enters the overview band. Inward trackpad and keyboard input immediately measures movement against that uncompressed overview scale until it leaves the band, then returns continuously to ordinary multiplicative zoom. This prevents the compressed maximum-distance scale from making the first inward trackpad events appear motionless. Native two-touch pinch retains its exact touch-distance ratio. Fit galaxy and resize recompute the same viewport envelope. Midpoint anchoring remains exact throughout both directions.
...
Active pointer movement now reuses viewport bounds captured at gesture start and mutates only the two resident touch records. Hover coalescing retains one scalar coordinate pair. Safari trackpad pinch stores the previous cumulative gesture scale and midpoint as scalars. Each `gesturechange` applies only the incremental ratio, so reversing inward responds on the first event instead of repaying cumulative outward gesture movement. While the camera remains inside the compressed overview band, that inward ratio advances at the uncompressed boundary velocity. The handler performs no point allocation or viewport geometry read.

The practical outer target now sits two percent beyond the padded fitted overview instead of ten percent beyond it, while resistance begins at 1.8 times fitted scale. The rational ceiling uses a 0.4-power response that keeps the boundary derivative continuous but increases braking deeper in the band. Wheel, touch, Safari, and keyboard zoom still use the same asymptotic outward curve. Trackpad and keyboard inward ratios respond from the first event at the uncompressed boundary velocity and rejoin the normal multiplicative curve continuously. Native two-touch inward ratios remain physically exact.

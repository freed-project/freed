...
The detached laboratory now starts through a one-shot worker with no main-thread graph compiler fallback. It transfers seventeen numeric scene, interaction-index, and direct-upload star buffers, retains all 30,000 semantic stars, and caps rich UI metadata at 192 nodes. Raw WebGPU resolves eleven active-theme colors in a fixed shader uniform block, so theme changes do not traverse or reupload resident stars. Product integration must remove the current `runAtlasOnMainThread` timeout path before cutover. Worker pressure must retain the last-good scene or enter a bounded compatibility state instead of rebuilding the complete galaxy in React.
...
- [x] Detached startup compiles the complete stress scene and interaction index in a worker, transfers seventeen numeric buffers, caps rich metadata, and never rebuilds the graph on the main thread

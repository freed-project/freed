...
The product route now consumes the approved replacement behind a reduced React shell. It uses a constrained 3D galactic plane with stable semantic geography on `x` and `y`, relationship prominence on `z`, tight identity systems, provider sectors on the outer rim, a locked perspective camera, and a renderer-neutral typed scene protocol. Raw WebGPU is the preferred shared backend, WebGL2 is the compatibility backend, and native Metal remains gated on later device evidence.

The foundation compiles the complete semantic galaxy into compact typed buffers for stable 3D positions, semantic prominence, bounded activity and recency brightness, theme color roles, interaction flags, and indexed edges. The atlas worker caches the rich semantic model by source revision and transfers the full star buffers only when graph source data or layout dimensions change. Pan and zoom retain those star buffers and move only the locked camera. Viewport presentation requests return capped label, hit, and edge detail without semantic buffers. During motion, the worker client keeps one presentation request in flight and coalesces newer camera positions. The renderer rasterizes a bounded label candidate pool once, then performs current-camera projection, density selection, and collision admission on every changed frame. It uploads only the small label instance stream when the accepted roster changes. Hover and selection patch dynamic typed arrays in place without rebuilding positions. An imperative engine owns the locked perspective camera, one instanced procedural star pass, one instanced settled edge pass, one instanced glyph label pass, depth-aware projected picking, theme palette resolution, WebGL fallback selection, resize, scene synchronization, rendering, and disposal. The React shell owns product props, worker lifecycle, interaction orchestration, and product callbacks. The complete approved vision, interaction contract, rendering contract, and staged roadmap live in [FRIENDS-GALAXY-ARCHITECTURE.md](./FRIENDS-GALAXY-ARCHITECTURE.md).
...
The detached laboratory and shipping product route start through a worker with no main-thread graph compiler fallback. They transfer twenty-one numeric scene, interaction-index, spatial-pick, and direct-upload star buffers, retain all 30,000 semantic stars in the stress envelope, and cap rich UI metadata at 192 nodes. Raw WebGPU resolves eleven active-theme colors in a fixed shader uniform block, so theme changes do not traverse or reupload resident stars. The worker-built sparse pick grid keeps pointer hit testing proportional to the camera corridor instead of the semantic star count, even when one pinned star is a distant outlier. WebGPU render density stays capped at 1.5 when settled and drops once at motion entry to 1.0 on compact canvases or 1.25 on wider canvases, without removing labels or stars. Worker pressure retains the last-good scene or enters a bounded compatibility state instead of rebuilding the complete galaxy in React.
...
The locked perspective camera and gesture mathematics are now shared product primitives too. Clip-safe near and far scale limits, full-canvas inset-aware fitted scale, one derived camera-frame state, compact initial framing, framed transforms, prominence-safe focus, overview, middle, and close detail thresholds, WebGPU view and motion uniforms, directional outward resistance, native-speed inward reversal, midpoint-preserving pinch, focal-point zoom, and Mac wheel normalization all live under shared UI source. One imperative navigation controller now composes those primitives and owns fit, stable-ID depth focus, pan, fixed-focal zoom, moving-focal Safari zoom, pinch, resize reanchoring, source replacement, and the live transform. Fit All now interpolates the camera's world center and logarithmic scale through one interruption-safe 420 ms curve, lands on the exact current fitted transform, and preserves immediate snapping for reduced motion. The detached laboratory and product engine both consume that controller. The laboratory no longer mutates camera coordinates or duplicates fit, focus, pinch, zoom, and resize math, and product cutover does not need a second camera curve.
...
The complete renderer palette now has the same boundary. Shared UI source owns background, surface, primary text, muted text, friend, connection, account, feed, selection, and fixed provider colors. It also owns the complete Starship, Dark Star, Scriptorium, Neon, Midas, and Ember renderer palette map keyed by the existing Freed theme ids. A shared semantic resolver reads only the typed scene kind and provider arrays, so Three.js color buffers and avatar seeds no longer call a laboratory helper. The backend alias accepts the shared scene and palette contracts directly. The laboratory retains only synthetic label metadata.

Bounded presentation assembly now lives behind one shared node-presentation resolver too. Product or laboratory code supplies only a label, initials, and priority for a typed scene node. Shared UI source owns visible-person ranking, compact and detail caps, selected-person retention, linked-account parent resolution, depth-aware viewport admission, label collision spacing, avatar exclusion, label and avatar atlas construction, and fixed-capacity interaction-overlay packing. Label texture construction is separate from current-camera label admission, so movement does not rerasterize text. Raw WebGPU and the WebGL2 compatibility path consume this same bounded selection contract. Synthetic identity names stay in the fixture, while no backend imports a laboratory presentation helper.
...
One shared renderer host now owns the active scene, theme palette, dimensions, density, animation preference, camera-motion state, provider-field style, selection, hover, detail, camera transform, bounded viewport presentation, and incremental activity patches. Every preferred-backend activation and compatibility recovery receives that complete state before its canvas becomes visible. Live activity revisions write only their sparse node batch. A typed recovery journal accumulates the latest state for each changed node and materializes a cumulative snapshot only when a new backend initializes. A viewport presentation update replaces only bounded rich nodes, labels, and provider regions. Motion updates refresh the candidate label pool without replacing semantic stars, the sparse interaction index, the camera, edges, or avatars. Settle additionally admits the bounded avatar roster. Raw WebGPU source replacement stages new scene buffers on the active device, then re-records bundles while preserving the canvas, context, device, and compiled pipelines. Pipeline-incompatible dust-mode changes, device loss, and unsupported compatibility backends retain the atomic full-activation path. The detached shell routes rendering, picking, palette changes, interaction, simulated loss, and disposal through the host while keeping asynchronous avatar image admission outside it. The product can therefore replace a failed WebGPU canvas without presenting a default theme, stale selection, wrong density, or partially initialized detail tier.

The detached product worker boundary now has separate source, viewport-presentation, and sparse-activity lanes. A source revision builds the real identity atlas at one deterministic virtual layout size, compiles every person and channel into the resident GPU scene, creates stable social and RSS activity bindings, validates the exact twenty-one-buffer receipt, and transfers that envelope once. Later presentation requests reuse the cached model and return only capped viewport nodes, real labels, and provider regions with no semantic buffer transfer. Motion requests are bounded and coalesced. Activity requests reuse the cached bindings and return six typed buffers for only affected semantic nodes, plus bounded unknown-source keys that tell the product shell when a structural source refresh is required. Source, presentation, and activity revisions are independently checked, missing or stale source revisions fail inside the worker, and error text is bounded. Product renderer scenes explicitly use the admitted atlas as their label and avatar candidate source, so neither motion, settle, nor activity handling rescans all resident people on the main thread. The synthetic laboratory can still use its complete resident fixture for visual stress work.

The matching browser-side product worker client now owns one generation-safe resident worker port. Graph-specific equality excludes Person notes and outreach metadata, Account capture and roster timestamps, and RSS scheduler state from structural invalidation. Activity recency stays in the sparse activity-summary lane. Isolated structural changes start immediately, while a bounded latest-wins scheduler collapses sustained background hydration and flushes at a fixed maximum wait. If another source arrives while the worker is compiling, the client retains only the latest queued source and admits no intermediate scene. Presentation requests are held until the source scene passes admission, limited to one in flight, and coalesced to the latest queued viewport. The input controller samples a changed moving camera at a bounded cadence, while the renderer still projects and admits labels against every camera frame. Sparse activity requests use an independent one-in-flight lane whose queued revisions merge by source key, so a newer update replaces only the same source and cannot discard unrelated deltas. Activity replies are applied in revision order. A superseded presentation reply is admitted but not applied. Source, presentation, and activity lanes have independent scalar deadlines consumed by the existing health poll. Current malformed messages, deserialization failures, worker errors, posting failures, and deadline failures terminate the active port and emit one bounded failure record. There is no browser timer in the gesture path and no caller-side compiler callback.
...
The practical outer target now equals the padded fitted overview, while resistance begins at 1.8 times fitted scale. Fit galaxy therefore lands on the same per-canvas ceiling approached by wheel, touch, Safari, and keyboard zoom. The rational ceiling uses a 0.15-power response that keeps the boundary derivative continuous and brakes more decisively deeper in the band. Every inward ratio responds on the first event at ordinary multiplicative speed, including a reversal that begins almost at the outer target. Keyboard and native two-touch inward ratios remain exact.
...
The allocation-free perspective projection helper now lives under shared UI source too. It projects a world point into a caller-owned two-value target through the shared view-projection matrix and reports bounded viewport admission without allocating screen vectors. Live labels, settled avatars, shell menu anchors, and functional projection tests consume the same helper, so the product has one depth-aware screen coordinate contract for presentation and overlay placement.

The billboard compositors now live under shared UI source behind renderer-neutral seed and palette contracts. Product metadata supplies a resolver for label, initials, and priority. Shared presentation assembly creates a bounded rasterized label pool, then chooses a current-camera visible roster independently from settled avatar admission. The shared label compositor owns avatar exclusion, the stronger theme-background outline, glyph measurement, texture packing, UV generation, and the eleven-float instance stream. The shared avatar compositor owns circular image clipping, initials fallback, selected borders, texture packing, UV generation, and the matching instance stream. Product metadata can replace synthetic metadata with real identities without rewriting candidate ranking, collision, selection retention, or GPU presentation assets.
...
Raw WebGPU receives animation and camera-motion state through its imperative backend contract. Semantic stars always use the same complete luminous profile and fixed intensity. They do not twinkle. Active motion changes only decorative-star density, render density, and provider-field shader cost. It exits the field shader through one coarse noise sample and a bounded envelope. Only stream variations add one spiral phase. Settled rendering restores the complete decorative field plus seven independent four-octave field samples, domain warping, wisps, and both spiral phases. The sign bit of the existing camera-scale uniform carries motion state, so no uniform block, bind group, pipeline, semantic geometry, label texture, or resident-star rebuild occurs. Every semantic star and billboard label remains resident and visible in both states.
...
Raw WebGPU semantic stars now use one retained four-vertex quad and one luminous transfer function during movement and rest. The prior moving octagon and simplified fragment profile were removed after installed production acceptance showed that their compositing changed apparent star intensity during drag. No geometry rebuild, draw-count mutation, semantic twinkle, or resident-star change occurs at the motion boundary.

The direct-upload star instance and geometry contracts now live under shared UI source. The product worker and raw backend share the exact eight-float semantic and decorative instance layout, stable semantic and provider palette roles, one four-vertex semantic quad for every camera state, and the 50,000-star decorative motion cap. The detached fixture compiler, worker envelope admission, interaction overlay encoder, palette uniforms, raw backend, and functional tests all import those shared constants and builders. The product does not translate worker output into a second GPU star format.
...
The three concrete renderer implementations now live under shared UI source too. One shared lazy factory resolves the requested renderer ID and injects the node-presentation resolver only into the WebGPU implementations that need bounded label and avatar metadata. The detached laboratory imports this factory and owns no renderer class or dynamic renderer path. The product can therefore instantiate Raw WebGPU, Three.js WebGPU, or the WebGL2 compatibility renderer from the real Friends engine without reaching into laboratory source or eagerly adding the large Three.js WebGPU reference chunk to startup.
...
- `packages/ui/src/lib/friends-galaxy-product-worker-protocol.ts` - versioned real-source and bounded viewport-presentation request, response, transfer, and admission contract
...
- `packages/ui/src/lib/friends-galaxy-product-engine.ts` - imperative product worker, metadata, retained renderer, state replay, and health boundary
...
Panning and pinching never rebuild semantic nodes, edges, or regions. A resident bounded label pool follows the current camera every frame, reapplies the active zoom-density cap, and refreshes screen-space collision placement during movement without per-frame text rasterization. Coalesced worker presentation replies may replace the bounded label texture when the visible candidate source changes, without replacing the semantic scene.
...
| 8.81  | Freeze decorative twinkle and reduce procedural nebula octaves during camera motion without removing stars, labels, or GPU buffers                                                                                                                               | High       | Done        |
...
| 8.88  | Retire the simplified moving-star fragment path after installed acceptance proved that it changed apparent semantic-star intensity                                                                                                                               | High       | Done        |
...
| 8.91  | Retire moving-star octagon strips and use the same semantic quad geometry during movement and rest                                                                                                                                                               | High       | Done        |
...
| 8.148 | Add a bounded viewport-presentation atlas lane that refreshes labels and selected metadata without replacing resident semantic stars or camera state                                                                                                             | High       | Done        |
| 8.149 | Split the real product worker into deterministic source and viewport-presentation lanes with one resident transfer, stale-revision containment, and atlas-only label plus avatar candidate resolution                                                            | High       | Done        |
...
| 8.152 | Assemble the product worker client, bounded metadata resolver, and retained renderer host behind one imperative engine with atomic source replacement and last-good-scene preservation                                                                           | High       | Done        |
| 8.153 | Add a third sparse activity worker lane with per-source coalescing, independent health deadlines, six-buffer transfer, unknown-source refresh signals, and cumulative backend recovery replay                                                                    | High       | Done        |
| 8.154 | Keep hover and selection on the direct stable-ID sparse renderer path, send no worker message, and replay the latest interaction before backend commit                                                                                                           | High       | Done        |
| 8.155 | Add one shared imperative navigation controller for fit, depth focus, pan, directional zoom, moving-focal Safari zoom, pinch, resize reanchoring, and source replacement, then make the lab and product engine consume it                                        | High       | Done        |
| 8.156 | Add the shared product input controller for pointer, native touch, Safari gestures, trackpad pan and zoom inertia, long press, keyboard control, demand rendering, diagnostics, and mobile presentation suspension                                               | High       | Done        |
| 8.157 | Replace the shipping `FriendGraph` atlas and `IdentityGalaxyEngine` internals with the worker-only product engine, Raw WebGPU preference, retained WebGL2 recovery, direct interaction, and no main-thread compiler fallback                                     | High       | Done        |
| 8.158 | Add the full-frame Friends background layer beneath the primary sidebar while keeping all input, controls, menus, and detail rails in the bounded interaction lane                                                                                               | Medium     | Done        |
| 8.159 | Keep the same graph canvas, camera, selection, worker scene, and GPU resources resident while the mobile Details lens is open                                                                                                                                    | High       | Done        |
| 8.160 | Complete integrated product-route workflow, accessibility, PWA touch, fallback, visual, diagnostics, and production-build acceptance for the next-generation Friends Galaxy                                                                                      | High       | Done        |
| 8.161 | Move Freed Desktop graph and overview activity summaries into bounded authenticated SQLite aggregates, page selected-person timelines, and resolve selected-Friend map candidates independently of the current page                                              | High       | Done        |
| 8.162 | Keep semantic-star geometry, luminous transfer, and intensity identical during movement and rest across Raw WebGPU and compatibility rendering                                                                                                                   | High       | Done        |
...
| 8.168 | Add the shared native and PWA `person_detail_v1` SQLite point query for one source-fenced Person header with 64 binary-ordered tags, 20 stable recent reach-out events, no Account or FeedItem corpus, and a 512 KiB response ceiling | High       | Done        |
...
| 8.179 | Add compact source-fenced Person and Account graph pages plus one reusable RSS feed page across native and PWA SQLite, each capped at 128 roots and 2 MiB in binary primary-key order with no notes, contact fields, histories, or complete identity corpus sent to React. Account and RSS rows include indexed visible activity counts and latest activity times, with indexed RSS image fallback, so the final graph path needs no JavaScript FeedItem scan or separate whole-graph activity aggregate | High       | Done        |
...
- [x] The detached renderers and product shell select the same Starship, Dark Star, Scriptorium, Neon, Midas, and Ember renderer palettes directly from the active Freed theme id
...
- [x] One imperative product engine connects source and settled worker lanes to the retained renderer host, replays pre-start state, keeps the old source coherent until atomic replacement, retains the last visible scene after worker failure, and reports admitted-scene readiness separately from worker-port readiness
- [x] The product engine routes sparse activity revisions through an independently coalesced worker lane, applies only affected semantic nodes during live updates, reports unknown source keys for structural refresh, and replays cumulative typed activity state only when a backend initializes or recovers
- [x] Product hover and selection bypass every worker lane, update the resident stable-ID interaction index directly, and replay their latest sparse state before a switched or recovered backend becomes visible
- [x] One shared navigation controller owns the live camera transform for the detached lab and product engine, preserving full-background insets, clip reserve, native inward response, pinch midpoint, and the same world center across responsive resize and source replacement
- [x] Fit All animates pan and zoom through the shared demand-rendered camera loop, lands on the exact current fitted transform, yields immediately to direct input, and snaps without animation when reduced motion is active
- [x] The shipping Friends route delegates renderer, worker, camera, gesture, picking, settle, recovery, and presentation ownership to the imperative product engine with no whole-graph main-thread fallback
- [x] Pointer, one-finger touch, two-finger pinch, Safari gesture events, Mac trackpad pan, trackpad zoom inertia, pointer throws, keyboard input, long press, and mobile presentation suspension share one product input controller with no React movement state
...
- [x] Active pan and zoom keep a bounded billboard label pool resident, reproject and collision-admit the current zoom-density roster on every changed frame, and coalesce worker presentation updates without rebuilding semantic scene buffers
...
- [x] Outward wheel and pinch zoom reach the exact Fit galaxy scale and centered transform for the current canvas while retaining a deeper far-plane reserve, and wheel, Safari, touch, and keyboard input restore native inward speed on the first reverse event
...
- [x] The detached worker, raw backend, and product engine share one direct-upload star instance layout, semantic palette-role encoding, motion-invariant semantic geometry, and decorative motion cap
- [x] The fitted zoom ceiling equals the padded Fit galaxy overview and uses a decisive 0.15-power outward approach, every inward reversal immediately uses the ordinary multiplicative ratio, and native touch retains its exact distance ratio
...
- [x] Raw WebGPU semantic stars use the same quad geometry, luminous transfer function, and fixed intensity during movement and rest
- [x] Semantic star twinkle is disabled in Raw WebGPU, Three.js WebGPU, and WebGL2 while decorative stars retain optional theme-aware ambient motion

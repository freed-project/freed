...
The matching browser-side product worker client now owns one generation-safe worker port. A new source revision terminates the superseded generation while the renderer host may keep its last-good scene visible. Presentation requests are held until the source scene passes admission, limited to one in flight, and coalesced to the latest queued viewport. The input controller samples a changed moving camera at a bounded cadence, while the renderer still projects and admits labels against every camera frame. Sparse activity requests use an independent one-in-flight lane whose queued revisions merge by source key, so a newer update replaces only the same source and cannot discard unrelated deltas. Activity replies are applied in revision order. A superseded presentation reply is admitted but not applied. Late messages and errors from an old generation cannot terminate the current worker. Source, presentation, and activity lanes have independent scalar deadlines consumed by the existing health poll. Current malformed messages, deserialization failures, worker errors, posting failures, and deadline failures terminate the active port and emit one bounded failure record. There is no browser timer in the gesture path and no caller-side compiler callback.

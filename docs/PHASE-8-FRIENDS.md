...
The locked camera now derives a deep clip reserve from viewport height, perspective field of view, semantic scene depth, and the shared 20,000-unit far clip distance. It separately derives an outer interaction target from the current fitted galaxy bounds. Wheel pinch, Safari gesture events, native touch pinch, and keyboard zoom decelerate asymptotically toward that fitted target without reaching an abrupt stop or the deeper clip reserve. The directional transfer curve begins outward resistance at 1.8 times fitted scale and uses a firmer 0.25-power response as it enters the overview band. Inward trackpad input immediately uses a constant three-times log-space response until it leaves that band, then consumes any remaining input at ordinary multiplicative speed. This removes trackpad startup lag without changing predictable keyboard steps or losing input when one event crosses the boundary. Native two-touch pinch retains its exact touch-distance ratio. Fit galaxy and resize recompute the same viewport envelope. Midpoint anchoring remains exact throughout both directions.
...
The practical outer target now sits two percent beyond the padded fitted overview instead of ten percent beyond it, while resistance begins at 1.8 times fitted scale. The rational ceiling uses a 0.25-power response that keeps the boundary derivative continuous and brakes more decisively deeper in the band. Wheel, touch, Safari, and keyboard zoom still use the same asymptotic outward curve. Trackpad inward ratios respond from the first event with a constant three-times log-space gain until they leave the compressed band. A crossing event applies only the consumed accelerated portion before returning its remaining input to normal multiplicative zoom. Keyboard and native two-touch inward ratios remain exact.
...
- [x] The fitted zoom ceiling uses a decisive 0.25-power approach before the padded overview target, trackpad reversal leaves the compressed band with immediate constant-speed input and no boundary loss, and native touch retains its exact distance ratio

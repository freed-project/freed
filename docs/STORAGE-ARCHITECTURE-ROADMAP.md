...
| Mutation registry              | Five normalized commit paths   | FeedItem read, saved, archived, liked, and removal mutations use generated programs and the extracted native signature verifier to atomically commit the transaction, actor tip, normalized value or tombstone, exact receipt, replication outbox, bounded invalidation, and revision. Saved and archived share one convergent register. Removal cascades owned children and stale removal clocks cannot replace the winning tombstone. Tampering, lost writer admission, changed replay, stale clocks, and incomplete targets fail without partial projection writes | Implement every remaining named mutation and route each durable product call site through the native or signed-intent boundary      |

...
| Query registry                 | In progress                    | `feed_page_v1` executes generated normalized SQL from the shared catalog, and the PWA worker enforces its stable keyset cursor plus row and byte budgets through one closed typed query dispatch. The former feed-specific worker request is gone, arbitrary SQL is impossible, and future queries extend one discriminated request-response union                                                                 | Implement the remaining generated programs, add equivalent Rust dispatch, and route every Freed Desktop and PWA surface through it  |

...
| PWA SQLite                     | In progress                               | Official SQLite WebAssembly installs the generated schema through one bounded worker over `opfs-sahpool`, verifies exact contract identity, executes thirty-four generated typed query programs, commits signed follower intents, applies exact signed results, and stages, verifies, and atomically activates normalized checkpoint pages without arbitrary SQL or IndexedDB Library rows. Ordinary feed, Friends feed, Saved feed, signal counts, item detail, Library facets, Saved analytics, Map, Story Wall, selected timelines, and Friends graph sources use the same shared bounded adapters as Freed Desktop, with the PWA supplying only its worker query executor. Factory reset closes the worker and recursively removes its private OPFS pool | Activate synchronized normalized checkpoints in OPFS SQLite, bind remaining Friends workspace and maintenance paths, complete recovery and selective content, then pass physical iPhone durability tests |

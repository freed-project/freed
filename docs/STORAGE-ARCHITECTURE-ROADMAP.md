...
| Query registry                 | Eight normalized query paths   | Feed pages, background item pages, compact invalidations, facet summaries, preference nodes, item details, Person details, and ranged reader bodies execute generated normalized SQL through equivalent closed browser and native dispatch. Person detail returns one header with bounded tags and stable recent reach-out events, while Accounts and timeline cards remain separate page queries. Interactive feeds and background scans use source-fenced keyset cursors. Background scans traverse compact metadata by primary key, include hidden and archived records, and return at most 64 rows. The change feed pins an upper revision, returns no entity rows, and refuses gaps. Facets use maintained counters. Preferences return binary-ordered typed scalars and explicit container markers. Item detail returns compact metadata plus reader-body locators. Reader bodies return no more than 256 KiB from inline text or five content-addressed chunks. Arbitrary SQL is impossible | Implement the remaining generated programs and route every Freed Desktop and PWA surface through them                               |

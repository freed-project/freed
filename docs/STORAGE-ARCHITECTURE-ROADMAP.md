...
| Normalized synchronization     | Accepted follower round trip implemented | The v2 registry has stable registry-plus-primary-key identity, closed authority, actor, capability, root, and child payloads, exact actor chain continuation, exact binary64 fractions, bounded native export, shared generated import SQL, cross-runtime digest vectors, transactional native and browser activation, accepted-authority proof, foreign-key proof, and lossless chunk verification. Browser SQLite commits complete signed intents with sparse optimistic fields and settles exact authority-signed results through actor-scoped contiguous result cursors in one transaction. The native Primary now creates an accepted result inside the same immediate transaction as the verified operation, normalized materialization, actor tip, revision, receipt, replication outbox, and invalidation. It rechecks the active authority key, signs bounded canonical bytes, advances a durable per-actor result chain, returns exact stored bytes after response loss, and rolls the whole commit back on a late result fault. No shell record participates | Add closed native rejection and already-applied result production, bounded result paging, then integrate operation, intent, result, and manifest flows with the normalized checkpoint contract |

...
| Query registry                 | Four normalized query paths    | Feed pages, facet summaries, preference leaves, and item details execute generated normalized SQL through equivalent closed browser and native dispatch. Feed paging uses source-fenced keyset cursors. Facets use maintained counters. Preferences return binary-ordered scalar leaves. Item detail performs a primary-key lookup and returns compact metadata plus reader-body locators, never full bodies or catch-all state. Arbitrary SQL is impossible | Implement the remaining generated programs and route every Freed Desktop and PWA surface through them                               |

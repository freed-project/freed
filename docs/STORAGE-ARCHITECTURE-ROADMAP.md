...
| Query registry                 | In progress                    | `feed_page_v1` executes generated normalized SQL from the shared catalog through equivalent closed browser and native dispatch. Both paths enforce the same source-fenced keyset cursor plus row and byte budgets, with an exact cross-runtime cursor vector. The former feed-specific worker request is gone, arbitrary SQL is impossible, and future queries extend the registered typed request and response unions | Implement the remaining generated programs and route every Freed Desktop and PWA surface through them                               |

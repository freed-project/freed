...
| Mutation registry              | First normalized commit path   | `feed_item_read_assignment` uses one generated program and the extracted native signature verifier to atomically commit the transaction, actor tip, normalized value, field clock, exact receipt, replication outbox, bounded invalidation, and revision. Tampering, lost writer admission, changed replay, and incomplete targets fail without partial writes                                                                 | Implement every remaining named mutation and route each durable product call site through the native or signed-intent boundary      |
| Query registry                 | In progress                    | `feed_page_v1` now executes generated normalized SQL in both TypeScript and Rust contract outputs, and the PWA worker enforces its stable keyset cursor plus row and byte budgets                                                                                                                                                                                                                        | Every Freed Desktop and PWA surface uses generated named SQL with stable keyset cursors and fixed budgets                            |

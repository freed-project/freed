...
| Final normalized schema        | Implemented, not active        | `normalized-schema-v1.sql` defines strict normalized authority, frontier, actor, capability, root, child, operation, causal-tip, replication, invalidation, signed-intent, result, optimistic-field, blob, chunk, trigger-maintained facet, and installation-local graph layout tables with bounded indexes and no shell or whole-record JSON authority. Device graph layout uses foreign-keyed Person and Account rows that are excluded from checkpoint and replication registries. Rust and TypeScript consume the exact same schema bytes, SHA-256 identity, and fixed application ID, and refuse foreign files before schema writes | Wire all native mutations and named queries to this catalog, then activate it through the one-epoch migration                        |
...
| Query registry                 | Twelve normalized query paths  | Feed pages, background item pages, compact invalidations, facet summaries, preference nodes, item details, Person details, Account details, compact Person, Account, and RSS feed graph source pages, and ranged reader bodies execute generated normalized SQL through equivalent closed browser and native dispatch. Detail queries return one source-fenced normalized row with bounded ordered child values. Graph source queries page compact roots by binary primary key without sending contact fields, notes, tags, histories, polling policy, or a complete identity corpus to React. Account and RSS graph rows carry indexed visible activity counts and latest activity times, so no JavaScript FeedItem scan or separate whole-graph activity summary is required. RSS images use the latest indexed visible item only when the feed has no image. Interactive feeds and background scans use source-fenced keyset cursors. The change feed pins an upper revision, returns no entity rows, and refuses gaps. Facets use maintained counters. Preferences return binary-ordered typed scalars and explicit container markers. Item detail returns compact metadata plus reader-body locators. Reader bodies return no more than 256 KiB from inline text or five content-addressed chunks. Arbitrary SQL is impossible | Implement the remaining generated programs and route every Freed Desktop and PWA surface through them                               |

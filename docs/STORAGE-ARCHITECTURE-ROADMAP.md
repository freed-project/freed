...
| PWA SQLite                     | In progress                               | Official SQLite WebAssembly installs the generated schema through one bounded worker over `opfs-sahpool`, verifies exact contract identity, executes the generated SQL registry, commits signed follower intents, applies exact signed results, and stages, verifies, atomically activates, and exports normalized checkpoint pages without arbitrary SQL or IndexedDB Library rows. Export pages pin one exact descriptor and enforce the same causal-frontier digest, registry, keyset cursor, 128-record, 1,048,576-response-byte, and 131,072-canonical-record contracts as native Rust. Feed, Saved, Friends, search, reader, Map, Story Wall, Header, Sidebar, command palette, settings catalogs, and maintenance scopes use bounded typed SQLite queries. Trigger-maintained facets supply exact navigation totals without scanning FeedItems. Desktop and PWA consume one shared provider-neutral follower coordinator. It reads one closed SQLite transport frontier, publishes one bounded normalized intent page, repairs one response-lost head from its immutable segment, and imports verified result segments through atomic SQLite callbacks. One browser-safe shared Google Drive transport factory owns enrollment publication and verification, exact certificate selection, normalized intent-head access, and bounded result-reference paging. The PWA binds its immediate, OAuth callback, 60-second, and manual sync passes to that coordinator without changing Google Drive behavior. The portable checkpoint store, IndexedDB read model, intent overlay, shell bootstrap, generic IndexedDB Library helper, dormant browser specifications, and their compatibility tests are deleted. IndexedDB appears only inside the dedicated nonextractable actor key vault because browser CryptoKey structured cloning cannot use SQLite. Explicit recovery removes the OPFS SQLite pool and that key vault after a second confirmation while preserving Google Drive authority                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Complete selective content transfer and physical iPhone durability proof |

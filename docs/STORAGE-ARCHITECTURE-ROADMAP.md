...
| Query registry                 | Three normalized query paths   | `feed_page_v1`, `library_facet_summary_v1`, and `preferences_snapshot_v1` execute generated normalized SQL through equivalent closed browser and native dispatch. Feed paging enforces the same source-fenced keyset cursor plus row and byte budgets with an exact cross-runtime cursor vector. Facets read trigger-maintained counters and refcounts without scanning FeedItem rows. Preferences return binary-ordered normalized scalar leaves without a settings shell. Arbitrary SQL is impossible | Implement the remaining generated programs and route every Freed Desktop and PWA surface through them                               |

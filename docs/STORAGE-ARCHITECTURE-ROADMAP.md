...
| PWA SQLite                     | In progress                               | Official SQLite WebAssembly installs the generated schema through one bounded worker over `opfs-sahpool`, verifies exact contract identity, executes the twenty-four generated SQL programs in the thirty-five-query registry, commits signed follower intents, applies exact signed results, and stages, verifies, and atomically activates normalized checkpoint pages without arbitrary SQL or IndexedDB Library rows. Ordinary feed, Friends feed, Saved feed, signal counts, item detail, Library facets, Saved analytics, Map, Story Wall, selected timelines, Friends activity and locations, Friends graph sources, and the Friend editor use the same typed query executor as Freed Desktop. Trigger-maintained facet counters now supply exact Library, unread, archivable, sample-root, and bounded per-platform navigation totals without scanning FeedItems. RSS rename emits one field-level title assignment. Bulk RSS removal atomically freezes the exact OPFS SQLite scope before bounded signed removal transactions, without consulting React state. The portable checkpoint store, IndexedDB read model, intent overlay, shell bootstrap, and their compatibility tests are deleted. Factory reset removes the OPFS SQLite pool and the narrow nonextractable actor key vault | Complete follower cloud orchestration, paged feed and identity surfaces, recovery, selective content, and physical iPhone durability proof |

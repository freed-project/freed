...
- Worker lifecycle evidence treats `worker_init`, `worker_init_recovery`, and `worker_idle_terminated` as attribution-bearing events. Idle termination summaries use the fixed reasons `quiet_window`, `pending_request_retry`, and `request_timeout_cleanup`. A missing or unknown reason is inconclusive.
- The `app-memory-pressure-p95` metric is the nearest-rank p95 of `appMemoryPressureBytes` from dense native samples inside one credited app-alive, page-load, and renderer generation. It requires complete pressure limits and rejects duplicates, sparse sampling, source gaps, or mixed generations. Worker-init outcomes automatically use it as a 128 MiB non-regression guardrail. It is not a claim about worker-owned bytes.

...
- **Fail-closed media-vault recovery:** The production SQLite Library status command directly reconciles the closed legacy manifest into durable content-addressed blobs, logical backup evidence, and an idempotent receipt without removing legacy files
...
| 5.42 | Native legacy media-vault reconciliation and local Library Core blob store | High     |
...
- [x] Freed Desktop's production SQLite Library status path directly validates the legacy media-vault manifest, opens every source path without following links, streams bytes into the local Library Core content-addressed store, records explicit quarantine, commits the logical backup before its restart-safe receipt, and leaves every legacy file in place

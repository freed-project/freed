...
- **Local display scale controls:** The feed toolbar and Appearance settings expose local card density and interface zoom controls, including a 75% to 200% zoom slider that persists on the current device
...
- [x] Freed Desktop factory reset blocks new local writes and drains in-flight store migrations, read marks, provider capture, social outbox actions, RSS, reader hydration, content fetch, semantic classification, snapshots, media archiving, and model downloads before deletion. It clears encrypted credentials, local media and AI models, provider health, snapshots, article and geocoding caches, retry state, runtime diagnostics, shortcut, playlist, scraper window, and provider user agent state, disconnects paired relay clients, rotates the pairing token, clears the relay-held document, and rejects cleanup failures before deleting the local document. Legal acceptance, release channel, and installation identity remain installation state.

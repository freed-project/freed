...
- **Native terminal sync soaks:** Dev-channel installed builds can pick up local sync trigger files from the native process, wake the existing renderer sync bridge without stealing focus, and keep the renderer alive while the normal Facebook, Instagram, or LinkedIn refresh path runs.
- **Quiet installed startup:** Freed Desktop now keeps cold startup quiet when launched with `open -g`, so installed-build soaks can start the app without interrupting the primary workstation. Explicit Show, dock reopen, recovery retry, and other foreground actions still raise the app.
...
- [x] Installed Desktop cold startup now has a quiet presentation path for `open -g` so terminal-driven soaks can launch the app without force-activating it

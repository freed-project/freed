...
| 5.131 | Store the device-local Google Contacts ledger in normalized SQLite generations, including scalar contact roots, bounded email, phone, photo, and organization children, suggestions, Account links, sync metadata, and one atomic active-generation selector. The native core executes the same closed replay-safe mutations as the PWA worker and exposes bounded singleton status plus byte-capped building-generation match pages. Suggestion review queries and product cutover remain in progress | High       | 🚧 In Progress |

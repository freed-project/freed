...
| 5.131 | Store the device-local Google Contacts ledger in normalized SQLite generations, including scalar contact roots, bounded email, phone, photo, and organization children, suggestions, Account links, sync metadata, and one atomic active-generation selector. The native core executes the closed replay-safe mutations and exposes byte-capped singleton status, building-generation matching, active suggestions, and unmatched-contact keyset queries. Product cutover remains in progress | High       | 🚧 In Progress |

...
- [x] Keep normalized checkpoint publication on one dedicated SQLite read
...
| 5.81  | Expose a typed native normalized checkpoint descriptor and pinned page command that holds one dedicated SQLite read transaction across the complete bounded export. Desktop and headless Primary keep the shared native session separate from ordinary command connections, so mutations continue in WAL mode without changing the exported Library, epoch, writer, revision, frontier, or record count | High       | ✓ Complete |
...
| 5.161 | Delete the remaining 7,853-line historical journal implementation, authority helpers, shell materializers, schema upgrades, and private journal suites. Freed Desktop never creates or upgrades that database again. A retained closed predecessor range is available only through exact descriptor-bound read-only validation for one-time normalized migration, while an empty directory is ignored and fresh installation creates normalized SQLite only | High       | ✓ Complete |
...
| 5.194 | Recover Google Drive Library Core requests when Google invalidates an access token before its recorded expiry. The shared Desktop transport forces one single-flight OAuth refresh after a 401, retries that request exactly once with the fresh token, and uses the stored replacement for the remainder of the bounded sync pass. The dev-only native trigger can invoke the same manual Library sync path as the Settings control for installed-build verification | High       | ✓ Complete |
...
- [x] Desktop checkpoint publication materializes one connection-local pinned export index, batches up to 4,096 records inside the generated byte ceilings, caps concurrent native query workers at eight, and coalesces priority-index refresh into one visible state update per pass

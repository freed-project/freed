...
| 5.131 | Store the device-local Google Contacts ledger in normalized SQLite generations, including scalar contact roots, bounded email, phone, photo, and organization children, suggestions, Account links, sync metadata, and one atomic active-generation selector. The native core now executes the same closed replay-safe generation mutations and canonical digest contract as the PWA worker. Bounded review queries and product cutover remain in progress                                                    | High       | 🚧 In Progress |

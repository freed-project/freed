...
Scheduled writer-ineligible captures return local deferrals, including missing or unreadable admission, follower role, and retired writer authority. These checks issue no provider contact and retain the original due opportunity with the existing jittered eligibility retry. They do not change provider failures, pace, or manual capture results. No repair resets already persisted deadlines on installation. Installed recovery proof remains pending genuine Library writer admission.
...
The macOS session gate accepts typed `IOConsoleLocked` and legacy `CGSSessionScreenIsLocked` plist booleans. Explicit locked state wins when signals conflict; missing or malformed state remains ineligible. Fractional sampled deadlines round upward only at the native integer boundary, preserving the device ledger and preventing early contact. Due providers emit a bounded `provider_schedule_deferred` event when the session gate blocks them, with `screen_locked` or `session_state_unavailable` as the category. The event does not consume an interval or increment provider failures. Installed multi-day effectiveness remains pending verification.
...
| 7.27 | Facebook feed and story advertising admission boundary                  | ✓ Complete     |
...
- [x] Facebook feed and story placements pass a versioned advertising admission boundary before expansion, with verified ads excluded, uncertain evidence deferred, detector failures closed, and privacy-safe health counters

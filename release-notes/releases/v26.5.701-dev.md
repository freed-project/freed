...
Dense surfaces and mobile validation repairs plus feed pressure relief, dense Map headroom, and Friends validation stability

### Fixes

- Dense Map marker popups are now created only when a marker is clicked, instead of building popup DOM for every visible marker during mount
- Dense Map marker sets now render simplified initials-only chrome past the DOM marker limit, cutting paint work for 1,600-author views
...
- Feed cards now stop loading inline media and avatar images before WebKit memory pressure blocks social scrapes
- Dense Friends graph zoom and pan reuse the static node layer for 1,600 connection views instead of redrawing cropped node batches each frame
- Friends graph rendering now disables Pixi canvas antialiasing to reduce frame drops during dense zoom and pan
- Last-seen Map validation is more stable across linked friend, unlinked marker, and selected view checks

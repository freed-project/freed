...
- **Fingerprinting risk requires a stop sign:** Before implementing or enabling any feature that could increase provider fingerprinting or detection risk, stop and warn the user in plain language. Do not continue until the user explicitly confirms the risk is acceptable. This includes, but is not limited to, new authenticated WebView loads, background provider navigation, extra provider API calls, automatic reply or comment hydration, scripted scrolling or clicking, altered timing patterns, new cookies or headers, media preloads, canvas or WebGL behavior, device fingerprint masking, and anything that changes how often Freed contacts X, Facebook, Instagram, LinkedIn, or another third-party provider. The warning must name the provider, describe the new observable behavior, explain why it could make Freed easier to fingerprint, and offer the lowest-profile alternative. The canonical machine-readable list of provider-visible paths lives in `scripts/lib/provider-visible-paths.mjs`; add new provider surfaces there, not in per-script copies. The trusted publisher invokes `scripts/worktree-publish.sh` to enforce this stop sign at publish time. Provider-visible changes must be committed before approval, and the branch must remain clean. Store the approval JSON outside the repository and publish with `--provider-risk-approval-file <approval.json>`. The record must include `approvedBy`, a `control-task` approval source, the top-level provider union, one exact provider scope per path, observable behavior, fingerprinting risk, lowest-profile alternative, exact provider-visible path set, owner approval reference, approval and expiry times, and the full committed binary diff hash for `origin/<base>...HEAD`. Provider-specific paths must match their inferred provider. Approval cannot be future-dated, lasts at most seven days, and must be unexpired. Any diff change or path-set change requires a new record. Compute the packet digest with `node scripts/lib/provider-visible-paths.mjs --approval-digest <approval.json>`. The owner must authorize that exact digest on the referenced control task through the private one-time bootstrap flow. A file that merely claims `approvedBy: AubreyF` is not approval. The validated record is copied into the PR body. Provider-visible PRs remain draft until the CODEOWNER reviews the exact diff.
...
`node scripts/doctor.mjs` checks the machine before loop or worktree work: the pinned Node toolchain from `.nvmrc`, PATH `node` consistency, `gh` (presence, that it runs, and binary architecture on macOS), git credential helpers that point at missing binaries, `git`, `curl`, `python3`, `~/.freed/automation/`, and the installed publisher trust chain. The publisher check uses the fixed root-owned schema v2 config at `/Library/Application Support/Freed/trusted-publisher-host.json`. It validates exact config shape, ownership and modes, pinned file digests, designated broker signature, exact clean control commit, private state and public-key record, and non-secret Keychain item presence without reading the signing key. Use `--require-publisher` for release and promotion gates. The trusted host repeats all use-time checks before signing a capability. `worktree-add.sh`, `worktree-publish.sh`, and the nightly runner run the doctor automatically in warn-only mode; loops and CI gates should run `node scripts/doctor.mjs --strict` and stop on failures. A surprising `node`/`npm` path or a broken `gh` is a machine issue to fix before debugging the repo.
...
- publish feature work through the owner-configured absolute signed host broker path in `FREED_TRUSTED_PUBLISHER`. The broker must live outside the candidate worktree. It reads only the root-owned schema v2 config at `/Library/Application Support/Freed/trusted-publisher-host.json`, validates its designated signature, immutable pinned broker, control checkout, publisher scripts, Node and GitHub CLI, then derives the exact repo, worktree, branch, base, and canonical base commit. A `main` promotion or production release-prep mode also binds the exact validated head. Only the native broker reads the Ed25519 signing key from Keychain. It issues one signed 60 second capability, and never sends the key or a reusable credential to a child process. The trusted launcher consumes that capability once, acquires a target-scoped publisher lease with a fixed 30 minute lifetime, and gives the pinned candidate helper only that lease. Stop if any trusted broker, config, capability, launcher, or publisher lease check fails.
- rerunning the trusted publisher should update the existing draft PR body and title, and push a ready PR back to draft when the local branch changes underneath it. At closeout, publish non-provider work with `--ready` so a finished, validated PR is marked ready for review. Provider-visible PRs stay draft. After exact-head CODEOWNER review, the owner performs a separate authorized ready transition. Draft means "still iterating, blocked, or needs discussion", not "authored by automation"
...
- the trusted publisher refuses stray untracked files unless you stage them yourself first or pass `--include-untracked`

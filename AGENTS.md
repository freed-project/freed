...
When the owner grants authorization without naming a level, ask which level applies before relying on it for a consequential action. Give the numbered choices below with a short description. Do not ask again when the owner names a level, and do not seek separate confirmation for actions already included in that level and the stated task.
...
6. **Full task authority:** All actions in Levels 1 through 5 that are reasonably necessary to finish the stated task, including production releases, deployments, rollback, external-service changes, and post-release verification.

Authorization is scoped to the task in which it is granted. A higher level includes every lower level. If the owner changes the level, the newest explicit level controls. Do not ask again for actions included in the granted level. Level 6 does not authorize unrelated work, disclosure of secrets, destructive changes outside the task, external posts written as the owner, bypassing hard platform safety controls, or actions that require credentials or permissions that do not exist. If scope is unclear in a way that could materially affect data, providers, money, production, or other people, ask one focused scope question even at Level 6.

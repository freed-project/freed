...
- **Fingerprinting risk requires a stop sign:** Before implementing or enabling any feature that could increase provider fingerprinting or detection risk, stop and warn the user in plain language. This includes new authenticated WebView loads, background provider navigation, extra provider API calls, automatic reply or comment hydration, scripted scrolling or clicking, altered timing patterns, new cookies or headers, media preloads, canvas or WebGL behavior, device fingerprint masking, and anything that changes how often Freed contacts X, Facebook, Instagram, LinkedIn, or another third-party provider. The warning must name the provider, describe the new observable behavior, explain why it could make Freed easier to fingerprint, and offer the lowest-profile alternative. Gate 1 requires the owner to explicitly approve that exact behavior and risk before code. Broad permission such as "proceed with everything" is not provider approval. The canonical machine-readable provider-visible path list lives in `scripts/lib/provider-visible-paths.mjs`; add new provider surfaces there, not in per-script copies. Gate 2 begins only after the candidate diff is committed and clean. Store the exact-diff approval JSON outside the repository, compute its digest with `node scripts/lib/provider-visible-paths.mjs --approval-digest <approval.json>`, and ask the owner to confirm that exact digest in the current task. The signing-free path uses `approvalSource.kind: "owner-confirmation"`, a stable task or thread reference, and the same exact `authorizationDigest` in the approval JSON. This is cooperative governance. The file does not authenticate the owner, so publication depends on the explicit confirmation in the current task plus the independent CODEOWNER review. The stronger optional path uses `approvalSource.kind: "control-task"` and the signed broker to create machine-verifiable task authorization for the same digest. Both paths require `approvedBy`, the top-level provider union, one exact provider scope per path, observable behavior, fingerprinting risk, lowest-profile alternative, exact provider-visible path set, owner approval reference, approval and expiry times, and the full committed binary diff hash for `origin/<base>...HEAD`. Provider-specific paths must match their inferred provider. Approval cannot be future-dated, lasts at most seven days, and must be unexpired. Any diff or path-set change returns to Gate 2. Any material behavior change returns to Gate 1. Publish only with `--provider-risk-approval-file <approval.json>`. `scripts/worktree-publish.sh` validates either source, copies the record into the PR body, and keeps every provider-visible PR draft until the CODEOWNER reviews the exact diff.
